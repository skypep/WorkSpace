...
import helper.phone.toro.com.imageselector.adapter.FolderAdapter;
import helper.phone.toro.com.imageselector.adapter.ImageAdapter;
import helper.phone.toro.com.imageselector.entry.Folder;
import helper.phone.toro.com.imageselector.entry.Image;
import helper.phone.toro.com.imageselector.model.ImageModel;
import helper.phone.toro.com.imageselector.utils.DateUtils;
...
public class ImageSelectorActivity extends AppCompatActivity {
    ...
    private void setSelectImageCount(int count) {
        if (count == 0) {
            ...
            tvPreview.setText(getString(R.string.preview) + "(" + count + ")");
            if (isSingle) {
                tvConfirm.setText(getString(R.string.confirm));
            } else if (mMaxCount > 0) {
                tvConfirm.setText(getString(R.string.confirm) + "(" + count + "/" + mMaxCount + ")");
            } else {
                tvConfirm.setText(getString(R.string.confirm) + "(" + count + ")");
            }
        }
    }
...
//            Toast.makeText(this, "没有图片", Toast.LENGTH_LONG).show();
    ...
    private void showExceptionDialog(final boolean applyLoad) {
        new AlertDialog.Builder(this)
                ...
                .setMessage(getString(R.string.p_message))
