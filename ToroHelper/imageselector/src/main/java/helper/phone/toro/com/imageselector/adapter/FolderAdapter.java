package helper.phone.toro.com.imageselector.adapter;
...
import helper.phone.toro.com.imageselector.R;
...
public class FolderAdapter extends RecyclerView.Adapter<FolderAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if (images != null && !images.isEmpty()) {
            holder.tvFolderSize.setText(images.size() + mContext.getString(R.string.pic));
            Glide.with(mContext).load(new File(images.get(0).getPath()))
                    .apply(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE))
                    .into(holder.ivImage);
        } else {
            holder.tvFolderSize.setText("0" + mContext.getString(R.string.pic));
