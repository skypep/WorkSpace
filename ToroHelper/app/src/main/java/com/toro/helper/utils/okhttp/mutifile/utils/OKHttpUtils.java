package com.toro.helper.utils.okhttp.mutifile.utils;
...
import com.toro.helper.utils.okhttp.mutifile.helper.ProgressHelper;
...
public class OKHttpUtils {
    ...
    private static Request getToroRequest(String url, List<String> fileNames, ProgressListener uiProgressRequestListener) {
        Request.Builder builder = new Request.Builder();
        builder.url(url)
                .post(ProgressHelper.addProgressRequestListener(
                        OKHttpUtils.getToroRequestBody("files",fileNames),
                        uiProgressRequestListener));
        return builder.build();
    }

    private static Request getToroRequest(String url, String fileName, ProgressListener uiProgressRequestListener) {
        Request.Builder builder = new Request.Builder();
        builder.url(url)
                .post(ProgressHelper.addProgressRequestListener(
                        OKHttpUtils.getToroRequestBody("files",fileName),
                        uiProgressRequestListener));
        return builder.build();
    }
    ...
    private static RequestBody getToroRequestBody(String key,List<String> fileNames) {
        //创建MultipartBody.Builder，用于添加请求的数据
        MultipartBody.Builder builder = new MultipartBody.Builder();
        ...
        for (int i = 0; i < fileNames.size(); i++) { //对文件进行遍历
            ...
            builder.addFormDataPart("files", "image.png", fileBody);
        }
        return builder.build(); //根据Builder创建请求
    }

    private static RequestBody getToroRequestBody(String key,String fileName) {
    ...
    public static void doToroUploadFilesRequest(OkHttpClient client, String url, List<String> fileNames, ProgressListener uiProgressRequestListener, Callback callback) {
        Call call = client.newCall(getToroRequest(url, fileNames, uiProgressRequestListener));
        call.enqueue(callback);
    }

    public static void doToroUploadFileRequest(OkHttpClient client, String url, String fileName, ProgressListener uiProgressRequestListener, Callback callback) {
        Call call = client.newCall(getToroRequest(url, fileName, uiProgressRequestListener));
        call.enqueue(callback);
    }
