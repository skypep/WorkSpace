...
//import com.android.camera.module.prefs.PrefsFragment1;
//import com.android.camera.module.prefs.SettingsActivity1;
...
public class PhotoMenu extends MenuController
    ...
    private View mMirrorSwitcher;//add
    private View mFillinSwitcher;//add
    private View mFaceBeautySwitcher;//add
    private View mFlashLightLyt;
    private View mSettingLyt;
...
//        mCountDownSwitcher = ui.getRootView().findViewById(R.id.countdown_switcher);	// frankie, add
...
//        mFlashSwitcher = ui.getRootView().findViewById(R.id.flash_mode_switcher);
...
//        initNativeView(ui.getRootView());
    ...
    private void initNativeView(View rootView){
        mCountDownSwitcher = rootView.findViewById(R.id.countdown_switcher);
        mFlashSwitcher = rootView.findViewById(R.id.flash_mode_switcher);
        mFilterModeSwitcher = rootView.findViewById(R.id.filter_mode_switcher);
        mSettingMenu = rootView.findViewById(R.id.menu);
        mSettingMenu.setVisibility(View.VISIBLE);
    }

    private void initView(View rootView){
        mFlashLightLyt = rootView.findViewById(R.id.layout_top_flashlight);
        mSettingLyt = rootView.findViewById(R.id.layout_top_setting);
        mTopMenu = rootView.findViewById(R.id.layout_top_menu);
        mFillinSwitcher = rootView.findViewById(R.id.fillin_iv);
        mFlashSwitcher = rootView.findViewById(R.id.flash_mode_iv);
        mFilterModeSwitcher = rootView.findViewById(R.id.filter_mode_iv);
        mSettingMenu = rootView.findViewById(R.id.setting_iv);
        mMirrorSwitcher = rootView.findViewById(R.id.mirror_iv);
        mFaceBeautySwitcher = rootView.findViewById(R.id.face_beauty_iv);
        mHdrSwitcher = rootView.findViewById(R.id.hrd_mode_iv);
    ...
    private void updateTopMenuView(){
        if(mIsFrontCamera){
            mMirrorSwitcher.setVisibility(View.VISIBLE);
            mFillinSwitcher.setVisibility(View.VISIBLE);
            mFlashSwitcher.setVisibility(View.GONE);
            mHdrSwitcher.setVisibility(View.GONE);
        }else {
            mMirrorSwitcher.setVisibility(View.GONE);
            mFillinSwitcher.setVisibility(View.GONE);
...
//        if(modeTv.getText() != null && !modeTv.getText().equals("")){
//            modeTv.setVisibility(View.VISIBLE);
//        }else {
//            modeTv.setVisibility(View.GONE);
//        }
    }

    private void initFaceBeautyButton(View button){
        IconListPreference pref = (IconListPreference) mPreferenceGroup.findPreference(CameraSettings.KEY_CHUS_FACE_BEAUTY);
        if (pref == null) {
            return;
        }
//        updatePopupImageViewFaceBeauty((ImageView) button);
...
//                    if(pref.getValue().equalsIgnoreCase("enable")){
//                        modeTv.setVisibility(View.VISIBLE);
//                        modeTv.setText(R.string.chus_face_beauty_enable_hint);
////                    ToastUtils.showToast(mActivity,mActivity.getString(R.string.chus_face_beauty_enable_hint));
//                    }else if(pref.getValue().equalsIgnoreCase("disable")){
//                        modeTv.setVisibility(View.GONE);
//                        modeTv.setText("");
//                    }
//                }
                updatePopupImageViewFaceBeauty((ImageView)v);
    ...
    private void initSelfFlashButton(View button){
        IconListPreference pref = (IconListPreference) mPreferenceGroup.findPreference(CameraSettings.KEY_SELFIE_FLASH);
        if (pref == null) {
               return;
        }
        int[] iconIds = pref.getLargeIconIds();
    ...
    private void initMirrorButton(View button){
        ...
        updatePopupImageViewSelfieMirror((ImageView)button);
        button.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                // directly change the value
                ...
                int index = pref.findIndexOfValue(pref.getValue());
                if(pref.getValue().equalsIgnoreCase("enable")){
                    ToastUtils.showToast(mActivity,mActivity.getString(R.string.chus_self_flash_disable_hint));
                }else{
                    ToastUtils.showToast(mActivity,mActivity.getString(R.string.chus_self_flash_enable_hint));
                }
                CharSequence[] values = pref.getEntryValues();
                index = (index + 1) % values.length;
                pref.setValueIndex(index);
    ...
    private void initSetButton(View button){
    ...
    private void showSettingMenu(final View menuView){
        ...
        ImageView touchToPhotoView = (ImageView)mSettingLyt.findViewById(R.id.menu_touch_photo_iv);
        updatePopupImageViewTouch2TakePhoto(touchToPhotoView);
        ImageView countDownView = (ImageView)mSettingLyt.findViewById(R.id.menu_countdown_iv);
        initCountdownButton(countDownView);
        delay = 3000;
        final int[] ids = new int[]{ R.id.menu_touch_photo_iv,R.id.menu_countdown_iv, R.id.menu_more_iv,R.id.menu_setting_iv};
        View.OnClickListener onClickListener_setting = new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int id_ = v.getId();
                if(id_ == R.id.menu_touch_photo_iv) {
                    // directly change the value
                ...
                } else if(id_ == R.id.menu_countdown_iv) {
                     // directly change the value
                    ...
                    if(value.equals("3")){
                      ToastUtils.showToast(mActivity,mActivity.getString(R.string.chus_delay_timer_3s_hint));
                    }else if(value.equals("5")){
                        ToastUtils.showToast(mActivity,mActivity.getString(R.string.chus_delay_timer_5s_hint));
                    }else if(value.equals("10")){
                        ToastUtils.showToast(mActivity,mActivity.getString(R.string.chus_delay_timer_10s_hint));
                    }else {
                        ToastUtils.showToast(mActivity,mActivity.getString(R.string.chus_delay_timer_0s_hint));
                    }
                    ...
                    handler.removeMessages(0);
                    handler.sendEmptyMessageDelayed(0,delay);
                } else if(id_ == R.id.menu_more_iv) {//more
                    ...
                    mSettingLyt.setVisibility(View.GONE);
//                    if(modeTv.getVisibility() == View.VISIBLE){
//                        modeTv.setVisibility(View.GONE);
//                    }
//                    delay = -1;
//                    handler.sendEmptyMessageDelayed(0,delay);
                }else if(id_ == R.id.menu_setting_iv){
//                    mTopMenu.setVisibility(View.VISIBLE);
//                    mTopMenu.setAlpha(1.0f);
//                    mSettingLyt.setVisibility(View.GONE);
                    delay = 0;
        ...
        for(int i=0;i<ids.length;i++) {
            View iv = mSettingLyt.findViewById(ids[i]);
            if(iv != null) {
                iv.setOnClickListener(onClickListener_setting);
            }
        }
    }
...
//                CameraSettings.KEY_SELFIE_MIRROR,
        ...
        updateTopMenuView();
		initFlashlightButton(mFlashSwitcher);
        initFilterModeButton(mFilterModeSwitcher);
        initSetButton(mSettingMenu);
        initSwitchItem_hdr(mHdrSwitcher);
        initMirrorButton(mMirrorSwitcher);
        initSelfFlashButton(mFillinSwitcher);
        initFaceBeautyButton(mFaceBeautySwitcher);
        ...
        //    }
        //}
//		initCountdownButton(mCountDownSwitcher); // frankie, add
    ...
    public boolean handleBackKey() {
		Log.v("CAM_b", "PhotoMenu, handleBackKey");
	...
	/*public*/private void initSwitchItem_hdr(View switcher) {
//        switcher.setVisibility(View.GONE);
		final IconListPreference pref = (IconListPreference) mPreferenceGroup.findPreference(CameraSettings.KEY_CAMERA_HDR);
		if (pref == null)
			return;
//        updatePopupImageViewHDR((ImageView) switcher);
		int[] iconIds = pref.getLargeIconIds();
		int resid = -1;
		int index = pref.findIndexOfValue(pref.getValue());
		if (!pref.getUseSingleIcon() && iconIds != null) {
...
//		switcher.setVisibility(View.VISIBLE);
		...
		switcher.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				index = (index + 1) % values.length;
				pref.setValueIndex(index);
                updatePopupImageViewHDR((ImageView) v);
				((ImageView) v).setImageResource(((IconListPreference) pref).getLargeIconIds()[index]);
	...
	private void initCountdownButton(View button) {
		...
		button.setVisibility(View.VISIBLE);
		//mPreferences.add(pref);				// do not add into ,
		//mPreferenceMap.put(pref, button);
//		button.setOnClickListener(new OnClickListener() {
//			@Override
//			public void onClick(View v) {
//				if(true) { // frankie,
//					showPopupWindow_countdown(v);
//					return ;
//				}
//
//				// directly change the value
...
//		  int[] resIds = new int[] {R.id.iv01, R.id.iv02, R.id.iv03};
//		  View.OnClickListener onClickListener_ = new View.OnClickListener() {
//			  @Override
//			  public void onClick(View v) {
//				  int id_ = v.getId();
//				  int index = 0;
//				  if(id_ == R.id.iv01) {
//					  index = 0;
//				  } else if(id_ == R.id.iv02) {
//					  index = 1;
//				  } else if(id_ == R.id.iv03) {
//					  index = 2;
//				  }
...
//			  ImageView iv = (ImageView)contentView.findViewById(resIds[i]);
//			  if(iv != null) {
//				  //iv.setOnClickListener(onClickListener_);
//			  }
//		  }
    ...
    private void initFlashlightButton(View button) {
        ...
        button.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
            	if(true) {
//					showPopupWindow_flashlight(v);
                    animFlashLightLyt();
                    showFlashLightMenu(v);
					return ;
				}
                IconListPreference pref = (IconListPreference) mPreferenceGroup.findPreference(CameraSettings.KEY_FLASH_MODE);
                if (pref == null) {
    ...
    private void animSettingLyt(){
        mSettingLyt.setVisibility(View.VISIBLE);
        AnimatorSet set = new AnimatorSet();
        ObjectAnimator animator1 = ObjectAnimator.ofFloat(mSettingLyt, "translationX", -300f, 0f);
        ObjectAnimator animator2 = ObjectAnimator.ofFloat(mTopMenu, "alpha", 1.0f, 0f);
        set.play(animator1).with(animator2);
        set.setDuration(100);
        set.start();
    ...
    private void showPopupWindow_flashlight(final View view) {
        View contentView = LayoutInflater.from(mActivity).inflate(R.layout.popup_flashlight_1, null);
        final PopupWindow popupWindow = new PopupWindow(contentView, ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT, true);
...
//        }
        ...
        popupWindow.setBackgroundDrawable(mActivity.getResources().getDrawable(R.drawable.chus_popup_bg_left));
        int[] locationOnScreen = new int[2];
        view.getLocationOnScreen(locationOnScreen);
        Log.v(TAG, "locationOnScreen: x=" + locationOnScreen[0] + " y=" + locationOnScreen[1]);
        locationOnScreen[0] -= 50;
        locationOnScreen[1] += 100;
        //popupWindow.showAsDropDown(view);
        popupWindow.showAtLocation(view, Gravity.TOP | Gravity.START, locationOnScreen[0], locationOnScreen[1]);
    }
...
//        int[] resIds_3 = new int[]{R.id.iv01, R.id.iv02, R.id.iv03};
//        int[] resIds_4 = new int[]{R.id.iv01, R.id.iv02, R.id.iv03, R.id.iv04};
//        int[] resIds = resIds_3;
...
//        }
	...
	private SettingsFragment2.Listener mSettingsFragment2_listener = new SettingsFragment2.Listener() {
        ...
        public void onListPrefChanged(ListPreference pref) {
        	PhotoMenu.this.onSettingChanged(pref);
        }
    };
...
//
//    /*public*/ private void initSceneModeButton(View button) {
//        button.setVisibility(View.INVISIBLE);
...
//        });
//    }
//    /*public*/private void addSceneMode() {
//        final IconListPreference pref = (IconListPreference) mPreferenceGroup
...
//
//        int gridRes = 0;
//		//Log.v(TAG, "rotation:" + rotation); // frankie, always 0
...
//		//Log.v(TAG, "gridRes:" + gridRes + " vertical_grid:" + R.layout.vertical_grid + " horiz_grid:" + R.layout.horiz_grid);
//			// frankie, always vertical_grid
//        mUI.hideUI();
//        mPreviewMenuStatus = PREVIEW_MENU_ON;
...
//        mPreviewMenu = basic;
//    }
//    /*public*/private void updateSceneModeIcon(IconListPreference pref) {
//        int[] thumbnails = pref.getThumbnailIds();
...
//         changeFilterModeControlIcon(pref.getValue());
	...
	// frankie, 2017.07.12, sometimes filter button seems not show correct .
    private void updateFilterModeIcon(ListPreference scenePref, ListPreference hdrPref) {
        ...
        if ((notSame(scenePref, CameraSettings.KEY_SCENE_MODE, Parameters.SCENE_MODE_AUTO))
            ...
            //changeFilterModeControlIcon("none"); // frankie, rm 
		...
		buttonSetEnabled(mFilterModeSwitcher, true); // frankie, always open
    }
...
//         hideNativeTopControlsMenu();
    ...
    public void hideCameraControls(boolean hide) {
        ...
        if(hide && (mFlashLightLyt.getVisibility() == View.VISIBLE || mSettingLyt.getVisibility() == View.VISIBLE)){
            mFlashLightLyt.setVisibility(View.GONE);
            mSettingLyt.setVisibility(View.GONE);
            mTopMenu.setVisibility(View.VISIBLE);
            mTopMenu.setAlpha(1.0f);
        ...
        if(mIsFrontCamera){
            mMirrorSwitcher.setVisibility(status);
            mFillinSwitcher.setVisibility(status);
            mFlashSwitcher.setVisibility(View.GONE);
            mHdrSwitcher.setVisibility(View.GONE);
        }else {
            mMirrorSwitcher.setVisibility(View.GONE);
        ...
        mFaceBeautySwitcher.setVisibility(status);

//        if(modeTv.getText() == null || modeTv.getText().equals("")){
//            modeTv.setVisibility(View.GONE);
//        }else {
//            modeTv.setVisibility(status);
//        }
        //if (TsMakeupManager.HAS_TS_MAKEUP) {
        //    mTsMakeupSwitcher.setVisibility(status);
        //} else {
			//mHdrSwitcher.setVisibility(status);
//			mCountDownSwitcher.setVisibility(status); // frankie, add
        //}
...
//                //mCameraSwitcher.setVisibility(status);
//				mCameraSwitcher.setVisibility(View.INVISIBLE); // frankie,
...
//        hideNativeTopControlsMenu();
	...
	private ListMenu.Listener mListMenuListener = new ListMenu.Listener() {
		...
		public void onSettingChanged(ListPreference pref) {
			//PhotoMenu.this.onSettingChanged(pref); // frankie, not callbacked , rm it 
		}
		@Override
		public void onPreferenceClicked(ListPreference pref, int y) {
			PhotoMenu.this.onPreferenceClicked(pref, y);
		}
		@Override
		public void onListMenuTouched() {
			PhotoMenu.this.onListMenuTouched();
		}
	};
