...
public class ZoomRenderer extends OverlayRenderer
	...
	private static final boolean ZOOM_NEW_STYLE_EN = true;
...
//    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(
			mCenterX - mMinCircle, mCenterY,
			mCenterX - mMaxCircle - 4, mCenterY, 
			mPaint);
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        if (mListener != null && circle != mCircleSize) {
            mCircleSize = circle;
            if (mCamera2) {
                float zoom = mZoomMinValue 
					+ (mZoomMaxValue - mZoomMinValue) / (mMaxCircle - mMinCircle) * (mCircleSize - mMinCircle);
    ...
    public void setOrientation(int orientation) {
		if(ZOOM_NEW_STYLE_EN) { // frankie, add 
			setOrientation_ext(orientation);
			return ;
		}
	...
	/////////////////////////////////////////////////////////////////////
	// frankie, 2017.07.11, add start
	public void onDraw_ext(Canvas canvas) {
        canvas.rotate(mOrientation, mCenterX, mCenterY);
        mPaint.setStrokeWidth(mInnerStroke);
        canvas.drawCircle(mCenterX, mCenterY, mMinCircle, mPaint);
        canvas.drawCircle(mCenterX, mCenterY, mMaxCircle, mPaint);
	...
	public boolean onScaleBegin_ext(ScaleGestureDetector detector) {
        return true;
	}
	public boolean onScale_ext(ScaleGestureDetector detector) {
        return true;
	}
	public void onScaleEnd_ext(ScaleGestureDetector detector) {

	}
	public void setOrientation_ext(int orientation) {
        mOrientation = orientation;
        if (mOrientation == 90) mOrientation = 270;
        else if (mOrientation == 270) mOrientation = 90;
	}
	// frankie, 2017.07.11, add end
