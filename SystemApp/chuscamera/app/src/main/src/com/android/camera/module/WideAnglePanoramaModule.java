...
import com.android.camera.module.mosaic.JniMosaic;
import com.android.camera.module.mosaic.MosaicFrameProcessor;
import com.android.camera.module.mosaic.MosaicPreviewRenderer;
        ...
        //,SurfaceTexture.OnFrameAvailableListener
{
    ...
    public static final int DEFAULT_BLEND_MODE = JniMosaic.BLENDTYPE_HORIZONTAL;
    public static final int DEFAULT_CAPTURE_PIXELS = 1440 * 1080; // frankie // 960 * 720;
	public static final boolean CONFIG_PREVIEW_RENDERER_enableWarpedPanoPreview = true;
    ...
    private static final int MSG_UNVALID_DIALOG_RESET_TO_PREVIEW = 0x1000;
	private static String getMSGstring(int what) {
		if(what == MSG_LOW_RES_FINAL_MOSAIC_READY) { return "MSG_LOW_RES_FINAL_MOSAIC_READY"; }
		else if(what == MSG_GENERATE_FINAL_MOSAIC_ERROR) { return "MSG_GENERATE_FINAL_MOSAIC_ERROR"; }
		else if(what == MSG_END_DIALOG_RESET_TO_PREVIEW) { return "MSG_END_DIALOG_RESET_TO_PREVIEW"; }
		else if(what == MSG_CLEAR_SCREEN_DELAY) { return "MSG_CLEAR_SCREEN_DELAY"; }
		else if(what == MSG_RESET_TO_PREVIEW) { return "MSG_RESET_TO_PREVIEW"; }
        else if(what == MSG_UNVALID_DIALOG_RESET_TO_PREVIEW) { return "MSG_UNVALID_DIALOG_RESET_TO_PREVIEW"; }
		else { return "*** UNKNOWN ***"; }
	}
    ...
    //private Object mRendererLock = new Object();	// frankie, 2017.09.13, no need to protect , since all call occurs on ui thread 
    ...
    private String mTargetFocusMode = Parameters.FOCUS_MODE_INFINITY;
		//Parameters.FOCUS_MODE_CONTINUOUS_VIDEO; // frankie, // Parameters.FOCUS_MODE_INFINITY;

    //private PanoOrientationEventListener mOrientationEventListener;
	private OrientationEventListener mOrientationEventListener;	// frankie, add
    ...
    private boolean takePanoFirst = true;
...
//    }
    ...
    public void init(CameraActivity activity, View parent) {
        ...
        mOrientationEventListener = new OrientationEventListener(mActivity) {
	        ...
	        public void onOrientationChanged(int orientation) {
	            ...
	            if (oldOrientation != mDeviceOrientation
	                    && oldOrientation != OrientationEventListener.ORIENTATION_UNKNOWN) {
	                Log.v(TAG, "onOrientationChanged: " + mDeviceOrientation);
	                mPreviewLayoutChanged = true;
	                if (!mOrientationLocked)
	                    mUI.setOrientation(mDeviceOrientation, true);
	            }
        ...
        mMainHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
            	Log.v(TAG, "handleMessage:" + getMSGstring(msg.what));
                switch (msg.what) {
                    case MSG_LOW_RES_FINAL_MOSAIC_READY:
                        onBackgroundThreadFinished();
                        setFinalMosaic((Bitmap) msg.obj);
						
                        saveHighResMosaic();
                        break;
                    case MSG_GENERATE_FINAL_MOSAIC_ERROR:
                        Toast.makeText(mActivity,R.string.msg_generate_final_mosaic_error,Toast.LENGTH_SHORT).show();
                        onBackgroundThreadFinished();
                        if (mPaused) {
                    ...
                    case MSG_END_DIALOG_RESET_TO_PREVIEW:
                        ...
                        clearMosaicFrameProcessorIfNeeded();
                        break;
                    case MSG_UNVALID_DIALOG_RESET_TO_PREVIEW:
                        Toast.makeText(mActivity,R.string.msg_generate_final_mosaic_error,Toast.LENGTH_SHORT).show();
                        onBackgroundThreadFinished();
                        resetToPreviewIfPossible();
                        clearMosaicFrameProcessorIfNeeded();
    ...
    public void onResumeAfterSuper() {
		...
		Log.v(TAG, "  mThreadRunning: " + mThreadRunning);
		Log.v(TAG, "  isMosaicMemoryAllocated: " + mMosaicFrameProcessor.isMosaicMemoryAllocated());
        ...
        if(takePanoIntent() && takePanoFirst){
             mMainHandler.postDelayed(new Runnable() {
                @Override
                 public void run() {
                    Log.v("aaa","+++++++ wide onShutterButtonClick");
...
//    }
    ...
    public boolean takePanoIntent(){
        String action = mActivity.getIntent().getAction();
        return (CameraActivity.ACTION_VOICE_MODULE_CAMERA_TAKE_PA_PHOTO.equals(action));
    }
    ...
    private void resetToPreviewIfPossible() {
		...
		mActivity.setSlideSwitcherShow(true, false); // frankie, not this time to show 
    }

    private void setFinalMosaic(Bitmap bitmap) {
        Log.v(TAG, "bitmap width*height = " + bitmap.getWidth() + "*" + bitmap.getHeight()
                + " getCaptureOrientation()=" + getCaptureOrientation());
        mUI.setFinalMosaic(bitmap, getCaptureOrientation());
    }
    ...
    private void setupCaptureParams(Parameters parameters) {
        ...
        Log.d(TAG, "camera preview h = " + mCameraPreviewHeight + " , w = " + mCameraPreviewWidth);
        parameters.setPreviewSize(mCameraPreviewWidth, mCameraPreviewHeight);

        // frankie, start print current previewFpsRange
        List<int[]> frameRates__1 = parameters.getSupportedPreviewFpsRange();
        Log.v(TAG, "frameRates__1.size() = " + frameRates__1.size());
        for(int[] rates : frameRates__1) {
            Log.v(TAG, "  min/max:" + rates[Parameters.PREVIEW_FPS_MIN_INDEX] + "/" + rates[Parameters.PREVIEW_FPS_MAX_INDEX]);
        }
        int[] currentFrameRange = new int[2];
        parameters.getPreviewFpsRange(currentFrameRange);
        Log.v(TAG, "current frame range: " + currentFrameRange[0] + " / " + currentFrameRange[1]);
		// frankie, end

        List<int[]> frameRates = parameters.getSupportedPreviewFpsRange();
        int last = frameRates.size() - 1;
        last = 0; // frankie, use the first, that is minimum ranges !   15000/15000, this is fixed fps
        //last = 1; // frankie, use the first, that is minimum ranges !   20000/20000, this is fixed fps
        ...
        Log.v(TAG, "getAutoExposureLock(): " + parameters.getAutoExposureLock());
        int exposureMin = parameters.getMinExposureCompensation();
		int exposureMax = parameters.getMaxExposureCompensation();
		int exposureCurrent = parameters.getExposureCompensation();
		float exposureStep = parameters.getExposureCompensationStep();
		Log.v(TAG, "exposure min/max:" + exposureMin + "/" + exposureMax 
			+ ", cur:" + exposureCurrent
			+ " step:" + exposureStep);
        parameters.setExposureCompensation((int) (exposureStep*5));    // do nothing help for preview darken
        exposureCurrent = parameters.getExposureCompensation();
        Log.v(TAG, "  exposureCurrent: " + exposureCurrent);
        // frankie, 2017.07.26, add end
        

        mHorizontalViewAngle = parameters.getHorizontalViewAngle();
        mVerticalViewAngle =  parameters.getVerticalViewAngle();
		Log.v(TAG, "mHorizontalViewAngle:" + mHorizontalViewAngle + " mVerticalViewAngle:" + mVerticalViewAngle);
    }
    ...
    private void configMosaicPreview() {
        ...
        //final
        boolean enableWarpedPanoPreview = mActivity.getResources().getBoolean(R.bool.enable_warped_pano_preview);
		enableWarpedPanoPreview = CONFIG_PREVIEW_RENDERER_enableWarpedPanoPreview; // frankie, 
        ...
        resetToPreviewIfPossible();	// frankie, -> startCameraPreview
    }
        ...
        if (!aborted && !mThreadRunning) {
            ...
            runBackgroundThread(new Thread() {
                ...
                public void run() {
                    ...
                    SystemClock.sleep(1000); // frankie, to see the generating dialog
                    if (jpeg != null && jpeg.isValid) {
                        ...
                        mMainHandler.sendMessage(mMainHandler.obtainMessage(
                                MSG_UNVALID_DIALOG_RESET_TO_PREVIEW));//MSG_END_DIALOG_RESET_TO_PREVIEW
                    }
                }
            });
        }
	 ...
	 */
	/*public*/private MosaicJpeg generateFinalMosaic(boolean highRes) {
		int mosaicReturnCode = mMosaicFrameProcessor.createMosaic(highRes);
		if (mosaicReturnCode == JniMosaic.MOSAIC_RET_CANCELLED) {
			return null;
		} else if (mosaicReturnCode == JniMosaic.MOSAIC_RET_ERROR) {
    ...
    private Uri savePanorama(byte[] jpegData, int width, int height, int orientation) {
        if (jpegData != null) {
            ...
            String filepath = Storage.generateFilepath(filename,
                              PhotoModule.PIXEL_FORMAT_JPEG,Storage.PHOTO_TYPE_PANO);
	...
	/*public*/private void saveHighResMosaic() {
		runBackgroundThread(new Thread() {
			@Override
			public void run() {
				...
				if (jpeg == null) {  // Cancelled by user.
					...
					{ // frankie,
						long profile_ = System.currentTimeMillis();
                        Log.w(TAG, "saveHighResMosaic, orientation:" + orientation);
						Bitmap b = BitmapFactory.decodeByteArray(jpeg.data, 0, jpeg.data.length);
						if (b != null && orientation != 0) {
							Matrix rotateMatrix = new Matrix();
							rotateMatrix.setRotate(orientation);
							b = Bitmap.createBitmap(
								b, 0, 0, b.getWidth(), b.getHeight(),rotateMatrix, false);
							
							int a_width = b.getWidth();
							int a_height = b.getHeight();
							int a_size = a_width * a_height;
					        ByteArrayOutputStream o = new ByteArrayOutputStream(a_size);
					        b.compress(Bitmap.CompressFormat.JPEG, 100, o);
							jpeg = new MosaicJpeg(o.toByteArray(), a_width, a_height);
                            orientation = 0;
						}
                        profile_ = System.currentTimeMillis() - profile_;
                        Log.w(TAG, "    rotate profile_ = " + profile_);
					}
					
					final Uri uri = savePanorama(jpeg.data, jpeg.width, jpeg.height, orientation);
					Log.v(TAG, "saved uri:" + uri.toString());
					if (uri != null) {
					...
					mMainHandler.sendMessage(mMainHandler.obtainMessage(MSG_END_DIALOG_RESET_TO_PREVIEW));
				}
			}
		});
	...
	/*public*/private void reportProgress() {
		mUI.resetSavingProgress();
		Thread t = new Thread() {
			@Override
			public void run() {
				while (mThreadRunning) {
					final int progress = mMosaicFrameProcessor.reportProgress(true, mCancelComputation);
					try {
						synchronized (mWaitObject) {
							mWaitObject.wait(50);
						}
	...
	private SurfaceTexture.OnFrameAvailableListener OnFrameAvailableListener_ = new SurfaceTexture.OnFrameAvailableListener() {
			@Override
			public void onFrameAvailable(SurfaceTexture surface) {
				/* This function may be called by some random thread,
				 * so let's be safe and jump back to ui thread.
	...
	private void startCameraPreview() {
		...
		//synchronized (mRendererLock) {
			...
			if (mCameraTexture != null) { // frankie, 
				mCameraTexture.setOnFrameAvailableListener(OnFrameAvailableListener_);
			}
