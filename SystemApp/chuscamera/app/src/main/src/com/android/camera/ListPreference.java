...
public class ListPreference extends CameraPreference {
    ...
    public String getValue() {
        if (!mLoaded) {
            ...
            if(mKey.equals(CameraSettings.KEY_PICTURE_SIZE)) {  // frankie, debug
                Log.v(TAG, "mDefaultValues.length=" + mDefaultValues.length);
                for(int i=0;i<mDefaultValues.length;i++) { // == 1
                    Log.v(TAG, "  mDefaultValues[] = " + mDefaultValues[i]);    // == null
                }
