...
import com.android.camera.module.PhotoUI;
...
public class FaceView extends View
    ...
    public FaceView(Context context, AttributeSet attrs) {
        ...
        Log.v(TAG, "FaceView /" + android.os.Process.myTid());
    }
    ...
    public void onSurfaceTextureSizeChanged(int uncroppedWidth, int uncroppedHeight) {
        ...
        Log.v(TAG, "onSurfaceTextureSizeChanged: " + mUncroppedWidth + "x" + mUncroppedHeight);
    }

    public void setFaces(Face[] faces) {
        if (LOGV) Log.v(TAG, "Num of faces=" + (faces != null ? faces.length : "null"));
        Log.v(TAG, "setFaces /" + android.os.Process.myTid());
        if (mPause) return;
        if (mFaces != null) {
            
            for (int i = 0; i < mFaces.length; i++) {       // print out
                if(mFaces[i] instanceof ExtendedFace) {
                    Log.w(TAG, "instanceof ExtendedFace");
                }
                else {
                    Log.w(TAG, "NOT instanceof ExtendedFace");
                }
            }
        ...
        // frankie, // mFaces == null , || (faces.length > 0 && mFaces.length > 0)
        if (mStateSwitchPending) {
            mStateSwitchPending = false;
    ...
    protected void onDraw(Canvas canvas) {
        if (!mBlocked && (mFaces != null) && (mFaces.length > 0)) {
            ...
            for (int i = 0; i < mFaces.length; i++) {
                ...
                float rx_ = mRect.right - mRect.left;
                float ry_ = mRect.bottom - mRect.top;
                canvas.drawRoundRect(mRect, rx_/30, ry_/30, mPaint);  // frankie,
                ...
                if (mFaces[i] instanceof ExtendedFace) {
                    ...
                    if (face.getLeftRightGazeDegree() != 0
                        ...
                        Log.e(TAG, "LeftRight || TopBottomGazeDegree != 0");
                        double length =
