...
public class MyFrameLayoutVideo extends FrameLayout {
	//private final static String TAG = "MainActivity.MyFrameLayoutVideo";
	private final static String TAG = "CAM_VideoUI.MyFrameLayoutVideo";

    private final static boolean DEBUG_VIEW_EN = false; // true; // false; // this debug view is the same as the TextureView for see where it is.
	...
	private static final float ASPECT_RATION_3_2 = 3f/2f;
	private static final float ASPECT_RATION_352_288 = 352f/288f;
    ...
    static String getRendererMessageString(int what) {
        if(what == RENDERER_MSG_CREATE) {return "RENDERER_MSG_CREATE";}
        else if(what == RENDERER_MSG_SIZE_CHANGED) {return "RENDERER_MSG_SIZE_CHANGED";}
        else if(what == RENDERER_MSG_CAMERA_INFO_CHANGED) {return "RENDERER_MSG_CAMERA_INFO_CHANGED";}
        else if(what == RENDERER_MSG_BEAUTY_LEVEL_CHANGED) {return "RENDERER_MSG_BEAUTY_LEVEL_CHANGED";}
        else if(what == RENDERER_MSG_FILTER_TYPE_CHANGED) {return "RENDERER_MSG_FILTER_TYPE_CHANGED";}
        else if(what == RENDERER_MSG_FRAME_AVAILABLE) {return "RENDERER_MSG_FRAME_AVAILABLE";}
        else if(what == RENDERER_MSG_RENDER_BITMAP) {return "RENDERER_MSG_RENDER_BITMAP";}
        else if(what == RENDERER_MSG_SNAPSHOT_PREVIEW) {return "RENDERER_MSG_SNAPSHOT_PREVIEW";}
        else if(what == RENDERER_MSG_PREVIEW_BLOCK) {return "RENDERER_MSG_PREVIEW_BLOCK";}
        else if(what == RENDERER_MSG_GET_EGLCONTEXT) {return "RENDERER_MSG_GET_EGLCONTEXT";}
        else if(what == RENDERER_MSG_SET_FRAME_CALLBACK) {return "RENDERER_MSG_SET_FRAME_CALLBACK";}
        else if(what == RENDERER_MSG_GET_CAMERA_PREVIEW_INFO) {return "RENDERER_MSG_GET_CAMERA_PREVIEW_INFO";}
        else if(what == RENDERER_MSG_DESTROY) {return "RENDERER_MSG_DESTROY";}
    ...
    static String getMainMessageString(int what) {
        if(what == MAIN_MSG_CREATE) {return "MAIN_MSG_CREATE";}
        else if(what == MAIN_MSG_SIZE_CHANGED) {return "MAIN_MSG_SIZE_CHANGED";}
        else if(what == MAIN_MSG_DESTROY) {return "MAIN_MSG_DESTROY";}
        else if(what == MAIN_MSG_ON_BITMAP_RENDERED) {return "MAIN_MSG_ON_BITMAP_RENDERED";}
        else if(what == MAIN_MSG_ON_PREVIEW_SNAPSHOT) {return "MAIN_MSG_ON_PREVIEW_SNAPSHOT";}
	...
	private int mBeautyLevel_1 = 0;
	
    private static Object mCreateLock = new Object();
	private boolean mCreateFlag = false;
	
    private static Object mDestroyLock = new Object();
...
//                }
                ...
                if(mRenderBitmapCallback != null) {mRenderBitmapCallback.onBitmapRendered((Bitmap) message.obj);}
            }
            else if(message.what == MAIN_MSG_ON_PREVIEW_SNAPSHOT) {
                if(mPreviewSnapshotCallback != null) {mPreviewSnapshotCallback.onPreviewSnapshotRendered((Bitmap) message.obj);}
            }
        }
    }
    ...
    class RendererHandler extends Handler {
        ...
        public RendererHandler(Looper looper, int beauty_level) {
            super(looper);
        ...
        protected void adjustSize(int rotation, boolean flipHorizontal, boolean flipVertical, FloatBuffer vertexBuffer, FloatBuffer textureBuffer){
        ...
        public void handleMessage(Message message) {
            if (RENDERER_MSG_FRAME_AVAILABLE != message.what) {
                Log.v(TAG, "@" + getTid() + "/" + "RendererHandler.handleMessage(" + message.what + "):" + getRendererMessageString(message.what));
            }
            if (message.what == RENDERER_MSG_CREATE) {
                ...
                // wait for camera ready, then get the camera info
                synchronized (mCameraPreviewInfo) {
                    cameraPreviewInfo.width = mCameraPreviewInfo.width;
                    cameraPreviewInfo.height = mCameraPreviewInfo.height;
                    cameraPreviewInfo.orientation = mCameraPreviewInfo.orientation;
                    cameraPreviewInfo.is_front = mCameraPreviewInfo.is_front;
                }
...
//                }
                if(mRenderBlackgroundRequest && !mSnapshotPreviewRequest) {
                    ...
                    if(mSnapshotPreviewRequest) {
                        ...
                        // read directly from the draw buffer, sometimes got the all black bitmp !!!
...
//                        preview_snapshot_bitmap.copyPixelsFromBuffer(ib);
                        ...
                        Bitmap preview_snapshot_bitmap;
						if(false) {
                ...
                Bitmap returned_bitmap = renderPhoto(bitmap, message.arg1 > 0 ? true : false);
...
//                }
            ...
            if (RENDERER_MSG_FRAME_AVAILABLE != message.what)
            {
                Log.v(TAG, "@" + getTid() + "/" + "RendererHandler.handleMessage(" + message.what + ") done!");
            }
        }
        ...
        public EGLContext get__eglContext() {
            ...
            synchronized (__eglContext_get_lock) {
                try {__eglContext_get_lock.wait(3000);
                } catch (InterruptedException ex) {}
            }
            return __eglContext;
        }
        ...
        public CameraPreviewInformation getCameraPreviewInformation() {
            ...
            synchronized (__camera_preview_infomation_get_lock) {
                try {__camera_preview_infomation_get_lock.wait(3000);
                } catch (InterruptedException ex) {}
            }
        ...
        private MagicBeautyFilter beautyFilter = null;      //
        private Bitmap renderPhoto(Bitmap bitmap, boolean isRotated) { // frankie, video does not use it !!!
			...
			if(mBeautyLevel_r != beautyFilter.mBeautyLevel) {
				beautyFilter.mBeautyLevel = mBeautyLevel_r;
                beautyFilter.onBeautyLevelChanged();
			}
            ...
            if (filter == null) {
                beautyFilter.onDrawFrame_n_oes(textureId, vertexBuffer, textureBuffer);
            } else {
            
                //beautyFilter.onDrawFrame(textureId);
                //filter.onDrawFrame(mFrameBufferTextures[0], vertexBuffer, textureBuffer); // frankie, error 

				beautyFilter.initCameraFrameBuffer(width, height);			// create one fbo(attached with one texture)
				
				int id = beautyFilter.onDrawToTexture_non_oes(textureId);	// render the picture into this fbo
			...
			if(beautyFilter != null) {
				beautyFilter.destroyFramebuffers();
        ...
        private Bitmap drawPreviewSnapshot(int width, int height, int textureId, float[] matrix, boolean isRotated) {
            ...
            FloatBuffer vertexBuffer = ByteBuffer.allocateDirect(TextureRotationUtil.CUBE.length * 4)
            ...
            if (filter != null) {
                id = cameraInputFilter.onDrawToTexture(textureId);  // id size is (imageWidth, imageHeight) ; //
                if(id > 0) {
                	filter.onDrawFrame(id, vertexBuffer, textureBuffer);
				}
            } else {
    ...
    public MyFrameLayoutVideo(Context context) {
        super(context);
        onNewInitialize(context);
    }
    public MyFrameLayoutVideo(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        onNewInitialize(context);
    }
    public MyFrameLayoutVideo(Context context, AttributeSet attrs) {
        super(context, attrs);
        onNewInitialize(context);
    }
    public MyFrameLayoutVideo(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        onNewInitialize(context);
    }
    private void onNewInitialize(Context context) {
        Log.v(TAG, "onNewInitialize");
        mContext = context;
    ...
    private void addTextureView_1(Context context) {
        ...
        textureView.setSurfaceTextureListener(new TextureView.SurfaceTextureListener() {
            ...
            public void onSurfaceTextureAvailable(SurfaceTexture surfacetTexture, int width, int height) {
                Log.v(TAG, "@" + getTid() + "/" + "** +TextureView.onSurfaceTextureAvailable:"
                        + width + " * " + height );
                Log.v(TAG, "    mAspectRatio:" + mAspectRatio + " already:" + mIsTextureAvaiableAlready);
                ...
                if(mRendererThread == null) {
                    mRendererThread = new HandlerThread("RendererThread");
                    mRendererThread.start();
                    mRendererHandler = new RendererHandler(mRendererThread.getLooper(), mBeautyLevel_1);
                }
				synchronized (mCreateLock) {
                    mCreateFlag = false;
                }
            ...
            public boolean onSurfaceTextureDestroyed(SurfaceTexture surfacetTexture) {
                ...
                if(mRendererThread != null) {
                    ...
                    try{mRendererThread.join(500); } 
                    catch(InterruptedException ex) {Log.w(TAG, "mRendererThread quit timeout!");}
    ...
    public void setAspectRatio(float ratio) {
        Log.v(TAG, "setAspectRatio:" + ratio + " pre-ratio:" + mAspectRatio);
    ...
    public void layoutPreview(float ratio) {
        ...
        if(ratio == ASPECT_RATION_16_9) {
			Log.v(TAG, "  ASPECT_RATION_16_9");
            lp = new FrameLayout.LayoutParams((int) scaledTextureWidth,
                    (int) scaledTextureHeight, Gravity.CENTER);
        }
		else if(ratio == ASPECT_RATION_3_2) {
			Log.v(TAG, "  ASPECT_RATION_3_2");
            lp = new FrameLayout.LayoutParams((int) scaledTextureWidth,
                    (int) scaledTextureHeight, Gravity.TOP | Gravity.CENTER_HORIZONTAL);
		}
		else if(ratio == ASPECT_RATION_352_288) {
			Log.v(TAG, "  ASPECT_RATION_352_288");
            lp = new FrameLayout.LayoutParams((int) scaledTextureWidth,
                    (int) scaledTextureHeight, Gravity.TOP | Gravity.CENTER_HORIZONTAL);
			lp.setMargins(0,150,0,0);
		}
		else { // should add bottom margin	// 4/3,  2/1
			Log.v(TAG, "  ASPECT_RATION_ : 4/3, 2/1");
            lp = new FrameLayout.LayoutParams((int) scaledTextureWidth,
                    (int) scaledTextureHeight, Gravity.CENTER);
    ...
    public void setCameraInfo(int width, int height, int orientation, boolean is_front) {
        ...
        if (mRendererHandler != null) {
            ...
            synchronized (mCameraPreviewInfoSetupLock) {
                try {
                    mCameraPreviewInfoSetupLock.wait();
                } catch (InterruptedException ex) {
                }
            }
        }
    ...
    public static class CameraPreviewInformation {
        ...
        public CameraPreviewInformation(int w, int h, int o, boolean i) {width=w;height=h;orientation=o;isFront=i;}
    }
    public CameraPreviewInformation getCameraPreviewInformation() {
        if (mRendererHandler != null) {
