...
public class PieRenderer 
		//extends OverlayRenderer implements FocusIndicator 
		extends PieMenuRenderer // frankie, add
        {

    private static final String TAG = "PieRenderer";

	private static final boolean FOCUS_RECTANGLE_ENABLE = true; // frankie, add
	...
	// override parent OverlayRenderer
    @Override
    ...
    public boolean onTouchEvent(MotionEvent evt) {
    	if(true) { // frankie, disable the touch
			return false;
		}
    ...
    private void startAnimation(long duration, final boolean timeout, float fromScale, float toScale) {
        setVisible(true);
	...
	public void showSuccess(boolean timeout) {
		if(FOCUS_RECTANGLE_ENABLE) {
			ext_showSuccess(timeout);
			return ;
		}

		Log.v(TAG, "showSuccess, mState:" + mState + " timeout:" + timeout);
	...
	public void showFail(boolean timeout) {
		if(FOCUS_RECTANGLE_ENABLE) {
			ext_showFail(timeout);
			return ;
		}

		Log.v(TAG, "showFail, mState:" + mState + " timeout:" + timeout);
		if (mState == STATE_FOCUSING) {
			startAnimation(SCALING_DOWN_TIME, timeout, mStartAnimationAngle);
			mState = STATE_FINISHING;
			mFocused = false;
	...
	/*public*/private void clear__(boolean waitUntilProgressIsHidden) {
		if (mState == STATE_PIE) {
			return;
		}
		cancelFocus();
    ...
    /*public*/private void setCenter(int x, int y) {
        mPieCenterX = x;
        mPieCenterY = y;
    ...
    private void setCircle(int cx, int cy) {
        ...
        mDial.set(cx - mCircleSize + mInnerOffset,
	...
	////////////////////////////////////////////////////////////// ext 
	@Override // frankie, add for PieMenuRenderer
...
//    }
    ...
    public void setBlockFocus(boolean blocked) {
    	Log.v(TAG, "setBlockFocus, blocked:" + blocked);
        mBlockFocus = blocked;
        if (blocked) {
            clear();
        }
...
//        canvas.restore();
	...
	private void ext_showStart() {
		Log.v(TAG, "ext_showStart, mState:" + mState);
		if (mState == STATE_PIE) return;
		cancelFocus();
		//mStartAnimationAngle = 67;
		int range = getRandomRange();
	...
	private void ext_showSuccess(boolean timeout) {
		Log.v(TAG, "ext_showSuccess, mState:" + mState + " timeout:" + timeout);
		if (mState == STATE_FOCUSING) {
	...
	private void ext_showFail(boolean timeout) {
		Log.v(TAG, "ext_showFail, mState:" + mState + " timeout:" + timeout);
		if (mState == STATE_FOCUSING) {
            ext_startAnimation(EXT_SHOW_DOWN_TIME, timeout, extToTargetSize);
			mState = STATE_FINISHING;
			mFocused = false;
	...
	private void ext_clear__(boolean waitUntilProgressIsHidden) {
		if (mState == STATE_PIE) {
			return;
		}
		cancelFocus();
