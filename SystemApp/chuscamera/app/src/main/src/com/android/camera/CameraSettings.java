...
public class CameraSettings {
    ...
    public static final String KEY_PICTURE_RATIO = "pref_camera_pictureratio_key";	// zhangjing, add
    ...
    public static final String KEY_RESET_DEFAULT_SETTINGS = "pref_reset_default_settings";
    ...
    public static final String KEY_AUX_GRID = "pref_aux_grid_key"; // frankie, add
    public static final String KEY_CHUS_FACE_BEAUTY = "pref_chus_face_beauty_key"; // frankie, add
    public static final String KEY_CHUS_TOUCH_TO_TAKE_PHOTO = "pref_chus_touch_to_take_photo_key"; // frankie, add
    public static final String KEY_CHUS_FILTER_TYPE = "pref_chus_filter_type_key"; // frankie, 2017.08.03, add
    ...
    private static final HashMap<Integer, String> VIDEO_ENCODER_TABLE = new HashMap<Integer, String>();
    ...
    public CameraSettings(Activity activity, Parameters parameters, int cameraId, CameraInfo[] cameraInfo) {
        mContext = activity;
    ...
    private void initPreference(PreferenceGroup group) {
        ...
        ListPreference pictureRatio = group.findPreference(KEY_PICTURE_RATIO);// zhangjing, add 
        ListPreference whiteBalance = group.findPreference(KEY_WHITE_BALANCE);
        ListPreference chromaFlash = group.findPreference(KEY_QC_CHROMA_FLASH);
        ListPreference sceneMode = group.findPreference(KEY_SCENE_MODE);
        ListPreference flashMode = group.findPreference(KEY_FLASH_MODE);
        ListPreference focusMode = group.findPreference(KEY_FOCUS_MODE);
        IconListPreference exposure = (IconListPreference) group.findPreference(KEY_EXPOSURE);
        ...
        ListPreference videoHfrMode = group.findPreference(KEY_VIDEO_HIGH_FRAME_RATE);
        ...
        if (pictureRatio != null) {
            CharSequence[] picture_size_entry_values = pictureSize.getEntryValues();
            Log.v("zhang", "  picture_size_entry_values.length:" + picture_size_entry_values.length);
            for(CharSequence s_ : picture_size_entry_values) {
                Log.v("zhang", "  " + s_);
            }

            List<String> l = getSupportedPicRatios_c(picture_size_entry_values);
            if (l != null && l.size() > 0) {
                for (int i = 0; i < l.size(); i++) {
                    Log.v("zhang", "getSupportedPicRatios :" + l.get(i));
                }
            }
            filterUnsupportedOptions(group, pictureRatio, l);

            Log.v("zhang", "==================================getSupportedPicRatios :");
            //pictureRatio.print();
            Log.v("zhang", "pictureRatio , pictureRatio.getValue: " + pictureRatio.getValue());
            CharSequence[] entryValues = pictureRatio.getEntryValues();
            for(int j=0;j<entryValues.length;j++) {
                Log.v("zhang", "  " + entryValues[j]);
            }

            if(!verifyPictureSizeByRatio(pictureSize.getValue(), pictureRatio.getValue())) {
                Log.v("zhang", "verifyPictureSizeByRatio(" + pictureSize.getValue() + "," + pictureRatio.getValue() + ") failed!!!");
                CharSequence[] size_by_ratio = getPictureSizeEntryValuesConfinedByRatio(
					picture_size_entry_values, pictureRatio.getValue());
                Log.v("zhang", "size_by_ratio.length = " + size_by_ratio.length);
                for(CharSequence c_ : size_by_ratio) {
                    Log.v("zhang", "  " + c_);
                }
                Log.v("zhang", "pictureSize changed to :" + size_by_ratio[0].toString());
                pictureSize.setValue(size_by_ratio[0].toString());     // default set the first one
            }
            else {
                Log.v("zhang", "verifyPictureSizeByRatio(" + pictureSize.getValue() + "," + pictureRatio.getValue() + ") ok .");
            }
	...
	// zhangjing, add start
    private final static double RATIO_THRESHOLD = 0.03;
    private final static double[] prebuilt_ratio_values = new double[]{
        1.7777, 1.3333, 1.0,		// *** frankie, 2017.08.15, here strange , return prebuilt_ratio_values[] , result the double value changed !!!
        								// so i returned following String instead . 
    };
	private final static String[] prebuilt_ratio_strs = new String[] {
		"1.7777", "1.3333", "1.0"
	};
	public final static int getPictureRatioIndex(String ratio_value) { // 0, 1, 2
        for(int i=0;i<prebuilt_ratio_strs.length;i++) {
            if(ratio_value.equals(prebuilt_ratio_strs[i])) {
                return i;
            }
        }
        return -1;
    }
    public final static double getPictureRatioDoubleValue(String ratio_value) {
        int index = getPictureRatioIndex(ratio_value);
        if(index >= 0 && index <= 2) {
            return prebuilt_ratio_values[index]; // if returned ok !!!
        }
        return -1;
    }
    private static String getNearestRatioStr(double r) {
        for (int i = 0; i < prebuilt_ratio_values.length; i++) {
            double ra_min = prebuilt_ratio_values[i] - RATIO_THRESHOLD;
            double ra_max = prebuilt_ratio_values[i] + RATIO_THRESHOLD;
            if (r >= ra_min && r <= ra_max) {
                return prebuilt_ratio_strs[i];
            }
        }
        return null;
    }
    private static List<String> getSupportedPicRatios(List<Size> sizes) {
        ArrayList<String> list = new ArrayList<String>();
        double d = 0.0f;
        for (Size size : sizes) {
            d = (size.width * 1.0) / size.height;
            String ratio_str = getNearestRatioStr(d);
			Log.v("zhang", "  " + size.width + "*" + size.height + " = " + ratio_str);
            if (!list.contains(ratio_str) && ratio_str != null) {
                list.add(ratio_str);
            }
        }
        return list;
    }
	private static List<String> getSupportedPicRatios_c(CharSequence[] sizes) {
        ArrayList<String> list = new ArrayList<String>();
        double d = 0.0f;
        for (CharSequence c : sizes) {
            String picture_size = c.toString();
            String[] v = picture_size.split("x", 2);
            int width = Integer.parseInt(v[0]);
            int height = Integer.parseInt(v[1]);

            d = (double)(width * 1.0) / (double)(height*1.0);
            String ratio_str = getNearestRatioStr(d);
			Log.v("zhang", "  " + width + "*" + height + " = " + ratio_str);
            if (!list.contains(ratio_str) && ratio_str != null) {
                list.add(ratio_str);
            }
        }
    ...
    public static CharSequence[] getPictureSizeValuesConfinedByRatio(CharSequence[] pictureSize, String ratio_value) {
        ArrayList<CharSequence> returned_list = new ArrayList<CharSequence>();
        int ratio_index = getPictureRatioIndex(ratio_value);
        double target_ratio = getPictureRatioDoubleValue(ratio_value);
        if(ratio_index >= 0 && ratio_index <= 2) {
            for(CharSequence c : pictureSize) {
                String picture_size = c.toString();
                String[] v = picture_size.split("x", 2);
                int width = Integer.parseInt(v[0]);
                int height = Integer.parseInt(v[1]);
                double calculated_ratio = (double)(width*1.0)/(double)(height*1.0);
                if(isRatioMatched(calculated_ratio, target_ratio)){
                    returned_list.add(c);
                }
            }
        }
        if(returned_list.size() > 0) {
            return returned_list.toArray(new CharSequence[returned_list.size()]); // empty
        } else {
            return new CharSequence[0];
        }
    }
    public static CharSequence[] getPictureSizeEntryValuesConfinedByRatio(CharSequence[] pictureSize, String ratio_value) {
        ArrayList<CharSequence> returned_list = new ArrayList<CharSequence>();
        int ratio_index = getPictureRatioIndex(ratio_value);
        double target_ratio = getPictureRatioDoubleValue(ratio_value);
        if(ratio_index >= 0 && ratio_index <= 2) {
    ...
    public static CharSequence[] getPictureSizeEntriesConfinedByRatio(CharSequence[] entries, CharSequence[] pictureSize, String ratio_value) {
        if(entries.length != pictureSize.length) {
            throw new RuntimeException("*** invalid PictureSize settings *** ");
        }
        ArrayList<CharSequence> returned_list = new ArrayList<CharSequence>();
        int ratio_index = getPictureRatioIndex(ratio_value);
        double target_ratio = getPictureRatioDoubleValue(ratio_value);
        if(ratio_index >= 0 && ratio_index <= 2) {
            for(int i=0;i<pictureSize.length;i++) {
                CharSequence c = pictureSize[i];
                String picture_size = c.toString();
                String[] v = picture_size.split("x", 2);
                int width = Integer.parseInt(v[0]);
                int height = Integer.parseInt(v[1]);
    ...
    static boolean isRatioMatched(double c, double target) {
        if(c >= (target - RATIO_THRESHOLD) && c <= (target + RATIO_THRESHOLD)) {
            return true;
        }
        return false;
    }
    public static boolean verifyPictureSizeByRatio(String picture_size, String ratio_value) {
        int ratio_index = getPictureRatioIndex(ratio_value);
        if(ratio_index >= 0 && ratio_index <= 2) {
            String[] v = picture_size.split("x", 2);
            int width = Integer.parseInt(v[0]);
            int height = Integer.parseInt(v[1]);
            double calculated_ratio = (double)(width*1.0)/(double)(height*1.0);
            if(!isRatioMatched(calculated_ratio, getPictureRatioDoubleValue(ratio_value))) {
                return false;
            }
        }
        return true;
    ...
    public static int readPreferredCameraId(SharedPreferences pref) {
        String rearCameraId = Integer.toString(CameraHolder.instance().getBackCameraId());
