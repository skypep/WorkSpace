...
public class CameraControlsPhoto extends RotatableLayout {

    private static final String TAG = "CAM_ControlsPhoto";
    ...
    public CameraControlsPhoto(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public CameraControlsPhoto(Context context) {
        this(context, null);
    }
...
//            mSwitcher.setPressed(false);
            ...
            //} else {
				...
				mCountdownSwitcher.setPressed(false);
            //}
...
//        ((ModuleSwitcher) mSwitcher).enableTouch(enable);
        ...
        //} else {
			...
			mCountdownSwitcher.setEnabled(enable);
        //}
...
//        mSwitcher = findViewById(R.id.camera_switcher);
        ...
        //} else {
			...
			mCountdownSwitcher = findViewById(R.id.countdown_switcher);
        //}
...
//        mIndicators = findViewById(R.id.on_screen_indicators);
        ...
        mFlashSwitcher = findViewById(R.id.flash_mode_switcher);
    ...
    private void center__(View v, int l, int t, int r, int b, int orientation, int rotation, Rect result, int idx) {
...
//		  toIndex(mIndicators, w, h, rotation,			0, 6, INDICATOR_INDEX);
            //toIndex(mFrontBackSwitcher, w, h, rotation,	2, 0, FRONT_BACK_INDEX);	// frankie, rm
            toIndex(mFrontBackSwitcher, w, h, rotation, 4, 6, FRONT_BACK_INDEX); // frankie, changed
            ...
            //} else {
				...
				toIndex(mCountdownSwitcher, w, h, rotation, 3, 0, HDR_INDEX);
            //}
            ...
            toIndex(mFlashSwitcher, w, h, rotation, 0, 0, SCENE_MODE_INDEX);
        } else {
            //toIndex(mSceneModeSwitcher, w, h, rotation, 0, 0, SCENE_MODE_INDEX);
            toIndex(mFlashSwitcher, w, h, rotation, 0, 0, SCENE_MODE_INDEX);
            //toIndex(mHdrSwitcher, w, h, rotation, 1, 0, HDR_INDEX);
            toIndex(mCountdownSwitcher, w, h, rotation, 1, 0, HDR_INDEX);
            ...
            toIndex(mFrontBackSwitcher, w, h, rotation, 3, 7, FRONT_BACK_INDEX); // frankie, changed
...
//        }
    ...
    private void resetLocation(float x, float y) {
        ...
        //} else {
			...
			mCountdownSwitcher.setX(mLocX[idx1][HDR_INDEX] + x);
        //}
        //mSceneModeSwitcher.setX(mLocX[idx1][SCENE_MODE_INDEX] + x);
        mFlashSwitcher.setX(mLocX[idx1][SCENE_MODE_INDEX] + x);
...
//        mIndicators.setX(mLocX[idx1][INDICATOR_INDEX] - x);
        ...
        //} else {
			...
			mCountdownSwitcher.setY(mLocY[idx1][HDR_INDEX] + y);
        //}
        //mSceneModeSwitcher.setY(mLocY[idx1][SCENE_MODE_INDEX] + y);
        mFlashSwitcher.setY(mLocY[idx1][SCENE_MODE_INDEX] + y);
...
//        mIndicators.setY(mLocY[idx1][INDICATOR_INDEX] - y);
    ...
    public void setTitleBarVisibility(int status){
        ...
        //} else {
			...
			mCountdownSwitcher.setVisibility(status);
        //}
    }

	private void markVisibility() {
		Log.v(TAG, "markVisibility");
        ...
        //} else {
            ...
            if (mCountdownSwitcher.getVisibility() == View.VISIBLE) {
                mViewList.add(mCountdownSwitcher);
			}
        //}
...
//            mViewList.add(mMute);
	...
	private AnimatorListener outlistener = new AnimatorListener() {
		@Override
		...
		public void onAnimationEnd(Animator animation) {
			...
			//} else {
				...
				mCountdownSwitcher.setVisibility(View.INVISIBLE);
			//}
...
//			mMute.setVisibility(View.INVISIBLE);
    ...
    public void hideUI() {
        ...
        //} else {
			...
			mCountdownSwitcher.animate().cancel();
        //}
...
//        ((ModuleSwitcher) mSwitcher).removePopup();
        mShutter.animate().setListener(outlistener);
        resetLocation(0, 0);
        markVisibility();
		Log.v(TAG, "    rotation:" + rotation); // frankie, 
        switch (rotation) {
            case 0:
                ...
                //} else {
					...
					mCountdownSwitcher.animate().translationYBy(-mSize).setDuration(ANIME_DURATION);
                //}
...
//                mIndicators.animate().translationYBy(mSize).setDuration(ANIME_DURATION);
            ...
            case 90:
                ...
                //} else {
					...
					mCountdownSwitcher.animate().translationXBy(-mSize).setDuration(ANIME_DURATION);
                //}
...
//                mIndicators.animate().translationXBy(-mSize).setDuration(ANIME_DURATION);
		...
		mRemainingPhotos.setVisibility(View.GONE); // frankie, add
        mRefocusToast.setVisibility(View.GONE);
		Log.v(TAG, "- hideUI");
    }


	private AnimatorListener inlistener = new AnimatorListener() {
		@Override
		public void onAnimationStart(Animator animation) {
		}
		@Override
...
//    }
    ...
    private void layoutRemaingPhotos() {
		...
		ori_ = 0; // frankie, fix the position 
        ...
        int m = getResources().getDimensionPixelSize(R.dimen.remaining_photos_margin_1); // frankie, 
	...
	public void showRemainingPhotos_(boolean show) { // frankie, add
        for (int i = mRemainingPhotos.getChildCount() - 1; i >= 0; --i) {
            mRemainingPhotos.getChildAt(i).setVisibility(View.VISIBLE);
        }
		mRemainingPhotos.setVisibility(show ? View.VISIBLE : View.GONE);
	}
...
//                mMute,
    ...
    public void showCameraSettings() {
        ...
        //} else {
			...
			mCountdownSwitcher.setVisibility(View.VISIBLE);
        //}
