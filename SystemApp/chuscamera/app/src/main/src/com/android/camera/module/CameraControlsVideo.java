...
public class CameraControlsVideo extends RotatableLayout {

    private static final String TAG = "CAM_ControlsVideo";
...
//    private View mVideoShutter;
    ...
    private int mSize;
    private static final int WIDTH_GRID = 4; // frankie, // 5;
    private static final int HEIGHT_GRID = 8; // frankie, // 7;
    ...
    public CameraControlsVideo(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public CameraControlsVideo(Context context) {
        this(context, null);
    }
...
//        mIndicators = findViewById(R.id.on_screen_indicators);
		...
		if(AGlobalConfig.config_module_VIDEO_MODULE_mute_button_en == false) {
			mMute.setVisibility(View.INVISIBLE);
		}

    }
    private void center__(View v, int l, int t, int r, int b, int orientation, int rotation,
            Rect result, int idx) {
...
//		  toIndex(mFrontBackSwitcher, w, h, rotation,	2, 0, FRONT_BACK_INDEX);
            toIndex(mFrontBackSwitcher, w, h, rotation, 4, 6, FRONT_BACK_INDEX);
            ...
            if(AGlobalConfig.config_module_VIDEO_MODULE_mute_button_en) {
				toIndex(mMute, w, h, rotation, 2, 0, MUTE_INDEX);
			}
            toIndex(mMenu, w, h, rotation, 3, 0, MENU_INDEX);

            toIndex(mPreview, w, h, rotation, 0, 7, PREVIEW_INDEX);
            toIndex(mFrontBackSwitcher, w, h, rotation, 3, 7, FRONT_BACK_INDEX);
            toIndex(mPauseVideo, w, h, rotation, 3, 7, FRONT_BACK_INDEX);
        }
    ...
    public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int rotation = getUnifiedRotation();
        //adjustBackground(); // frankie, 2017.06.10, remove 
        ...
        l = 0;
        t = 0;
		Log.v(TAG, "l:" + l + " t:" + t + " r:" + r + " b:" + b); // frankie, 
        ...
        center__(mShutter, l, t, r, b, orientation, rotation, shutter, SHUTTER_INDEX);
...
//            toRight(mReviewDoneButton, shutter, rotation);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas); // frankie, add
//        if (mTopMargin != 0) { // frankie, remove
//            int rotation = getUnifiedRotation();
...
//        mIndicators.setY(mLocY[idx1][INDICATOR_INDEX] - y);
    ...
    public void setTitleBarVisibility(int status){
		Log.v(TAG, "setTitleBarVisibility:" + status);
...
//			mVideoShutter.setVisibility(View.INVISIBLE);
    ...
    public void setFrontBackSwitcherEnable(boolean b){
        mFrontBackSwitcher.setEnabled(b);
    }



    private static final int FADE_OUT_DURATION = 100;
	private AnimatorListener fadeOutlistener = new AnimatorListener() {
		@Override
		public void onAnimationStart(Animator animation) {
		}
		@Override
    ...
    public void hideUI() {
		if(true) {
			...
			mShutter.animate().setListener(fadeOutlistener); // frankie, add

            mFrontBackSwitcher.animate().alpha(0.0f).setDuration(FADE_OUT_DURATION);
            mHdrSwitcher.animate().alpha(0.0f).setDuration(FADE_OUT_DURATION);
            mSceneModeSwitcher.animate().alpha(0.0f).setDuration(FADE_OUT_DURATION);
            mShutter.animate().alpha(0.0f).setDuration(FADE_OUT_DURATION);
            mMenu.animate().alpha(0.0f).setDuration(FADE_OUT_DURATION);
            //mMute.animate().alpha(0.0f).setDuration(FADE_OUT_DURATION);
            mPreview.animate().alpha(0.0f).setDuration(FADE_OUT_DURATION);
            mShutter.animate().alpha(0.0f).setDuration(FADE_OUT_DURATION);
...
//        ((ModuleSwitcher) mSwitcher).removePopup();
        mShutter.animate().setListener(outlistener); // frankie, add
        resetLocation(0, 0);
        markVisibility();
        switch (rotation) {
            case 0:
...
//                mIndicators.animate().translationXBy(-mSize).setDuration(ANIME_DURATION);
	...
	private AnimatorListener fadeInlistener = new AnimatorListener() {
		@Override
		public void onAnimationStart(Animator animation) {
		}
		@Override
    ...
    public void showUI() {
		if(true) {
			...
			//mMute.animate().cancel();
			mPreview.animate().cancel();
			mShutter.animate().setListener(fadeInlistener); // frankie, add

            mFrontBackSwitcher.animate().alpha(1.0f).setDuration(FADE_IN_DURATION);
            mHdrSwitcher.animate().alpha(1.0f).setDuration(FADE_IN_DURATION);
            mSceneModeSwitcher.animate().alpha(1.0f).setDuration(FADE_IN_DURATION);
            mShutter.animate().alpha(1.0f).setDuration(FADE_IN_DURATION);
            mMenu.animate().alpha(1.0f).setDuration(FADE_IN_DURATION);
            //mMute.animate().alpha(1.0f).setDuration(FADE_IN_DURATION);
            mPreview.animate().alpha(1.0f).setDuration(FADE_IN_DURATION);
            mShutter.animate().alpha(1.0f).setDuration(FADE_IN_DURATION);
...
//        mIndicators.setVisibility(View.VISIBLE);
        ...
        mShutter.animate().setListener(inlistener);
        switch (rotation) {
            case 0:
...
//    }
    ...
    private void layoutRemaingPhotos() {
    	int ori_ = mOrientation;
		ori_ = 0; // frankie, fix its orientation 
        ...
        int m = getResources().getDimensionPixelSize(R.dimen.remaining_photos_margin_1);
        ...
        mRemainingPhotos.setRotation(-ori_);
    }
    ...
    public void setPreviewRatio(float ratio, boolean panorama) {
        if (panorama) {
            ...
            //mPreviewRatio = r;
            int mPreviewRatio = r;	// frankie, add 
...
//                mSwitcher,
    ...
    // frankie, 2017.07.12, add start
    private void showSnapshotButton() {

    }
    private void hideSnapshotButton() {

    }
    public void hidePreviewThumbnailButton() {
        ObjectAnimator alphaAnimator = ObjectAnimator.ofFloat(mPreview, "alpha", 1f, 0f);
        alphaAnimator.setDuration(300);
        alphaAnimator.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationCancel(Animator animation) {
                Log.v(TAG, "hidePreviewThumbnailButton,onAnimationCancel");
                mPreview.setVisibility(View.INVISIBLE);
                showSnapshotButton();
            }
            @Override
            public void onAnimationEnd(Animator animation) {
                Log.v(TAG, "hidePreviewThumbnailButton,onAnimationEnd");
                mPreview.setVisibility(View.INVISIBLE);
                showSnapshotButton();
            }
        });
    ...
    public void showPreviewThumbnailButton() {
        ObjectAnimator alphaAnimator = ObjectAnimator.ofFloat(mPreview, "alpha", 0f, 1f);
        alphaAnimator.setDuration(300);
        alphaAnimator.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationCancel(Animator animation) {
