...
class AndroidCameraManagerImpl implements CameraManager {
            ...
            post(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        //throw e;	// frankie,  add for debug // 2017.11.09, rm
                        Util.showErrDialog(CameraActivity.mActivityContext, "Error", "take picture failed.");
                    }
                }
            });
        }
        public void requestTakePicture__i(
                final ShutterCallback shutter,
                final PictureCallback raw,
                final PictureCallback postView,
                final PictureCallback jpeg, final TakePictureFatalErrorCallback_i cb) {
            post(new Runnable() {
                @Override
                public void run() {
                	Log.v(TAG, "+ requestTakePicture__i run");
                    //if(cb != null) {  cb.onTakePictureFatalError(mCamera); } // frankie, debug 
                    ...
                    try {
                        ...
                        //throw e;	// frankie,  add for debug // 2017.11.09, rm
                        //Util.showErrDialog(CameraActivity.mActivityContext, "Error", "take picture failed.");
                        if(cb != null) {  cb.onTakePictureFatalError(mCamera); }
                    }
					Log.v(TAG, "- requestTakePicture__i run");
    ...
    public class AndroidCameraProxyImpl implements CameraManager.CameraProxy {
        ...
        // frankie, 2017.12.15, start
        @Override
        public void takePicture__i(
                Handler handler,
                CameraShutterCallback shutter,
                CameraPictureCallback raw,
                CameraPictureCallback post,
                CameraPictureCallback jpeg,
                TakePictureFatalErrorCallback cb) {
            Log.v(TAG, "takePicture__i");
            mCameraHandler.requestTakePicture__i(
                    ShutterCallbackForward.getNewInstance(handler, this, shutter),
                    PictureCallbackForward.getNewInstance(handler, this, raw),
                    PictureCallbackForward.getNewInstance(handler, this, post),
                    PictureCallbackForward.getNewInstance(handler, this, jpeg),
                    TakePictureFatalErrorCallbackForward.getNewInstance(handler, this, cb)
                    );
        }
        // frankie, 2017.12.15, end
    ...
    // frankie, 2017.12.15, start
    private static class TakePictureFatalErrorCallbackForward implements TakePictureFatalErrorCallback_i {
        private final Handler mHandler;
        private final TakePictureFatalErrorCallback mCallback;
        private final CameraProxy mCamera;
         ...
         *                or null if any parameter is null.
         */
        public static TakePictureFatalErrorCallbackForward getNewInstance(
                Handler handler, CameraProxy camera, TakePictureFatalErrorCallback cb) {
            if (handler == null || camera == null || cb == null) return null;
            return new TakePictureFatalErrorCallbackForward(handler, camera, cb);
        }

        private TakePictureFatalErrorCallbackForward(
            Handler h, CameraProxy camera, TakePictureFatalErrorCallback cb) {
        ...
        @Override
        public void onTakePictureFatalError(android.hardware.Camera camera) {
            final android.hardware.Camera currentCamera = mCamera.getCamera();
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    if ((currentCamera != null) && currentCamera.equals(mCamera.getCamera())) {
                        mCallback.onTakePictureFatalError();
                    }
                }
            });
        }
