...
public class MediaSaveService extends Service {
    ...
    public void addMpoImage(final byte[] csImage,
        ...
        long size = (csImage == null ? 0: csImage.length) + bayerImg.length + monoImg.length;
        mMemoryUse += size;
        if (isQueueFull()) {
            onQueueFull();
        }
    ...
    public void addImage(final byte[] data, String title, long date, Location loc,
        ...
        addImage(data,title,date,loc,width,height,orientation,exif,l,resolver,pictureFormat,null);
    }

    public void addImage(final byte[] data, String title, long date, Location loc,
            int width, int height, int orientation, ExifInterface exif,
            OnMediaSavedListener l, ContentResolver resolver, String pictureFormat,String photoType) {
        if (isQueueFull()) {
            Log.e(TAG, "Cannot add image when the queue is full");
            return;
        }
        ImageSaveTask t = new ImageSaveTask(data, title, date,
                ...
                width, height, orientation, exif, resolver, l, pictureFormat,photoType);
    ...
    public void addImage(final byte[] data, String title, long date, Location loc,
        ...
        addImage(data, title, date, loc, 0, 0, orientation, exif, l, resolver,
                 PhotoModule.PIXEL_FORMAT_JPEG,null);
    }
    public void addImage(final byte[] data, String title, Location loc,
        ...
        addImage(data, title, System.currentTimeMillis(), loc, width, height,
                orientation, exif, l, resolver,PhotoModule.PIXEL_FORMAT_JPEG,null);
    }
    ...
    private class MpoSaveTask extends AsyncTask<Void, Void, Uri> {
        ...
        protected void onPostExecute(Uri uri) {
            ...
            long size = (csImage == null ? 0: csImage.length) + bayerImage.length + monoImage.length;
            mMemoryUse -= size;
    ...
    private class ImageSaveTask extends AsyncTask <Void, Void, Uri> {
        ...
        public ImageSaveTask(byte[] data, String title, long date, Location loc,
            ...
            this(data,title,date,loc,width,height,orientation,exif,resolver,listener,pictureFormat,null);
        }

        public ImageSaveTask(byte[] data, String title, long date, Location loc,
                             int width, int height, int orientation, ExifInterface exif,
                             ContentResolver resolver, OnMediaSavedListener listener, String pictureFormat,String photoType) {
            this.data = data;
            this.title = title;
            this.date = date;
            this.loc = loc;
        ...
        protected Uri doInBackground(Void... v) {
            ...
            return Storage.addImage(
                    resolver, title, date, loc, orientation, exif, data, width, height, pictureFormat,photoType);
        }
