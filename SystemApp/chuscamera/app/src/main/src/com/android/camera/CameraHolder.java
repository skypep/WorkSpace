...
public class CameraHolder {
    ...
    public static void injectMockCamera(Camera.CameraInfo[] info, CameraProxy[] camera) {
    ...
    public synchronized void release() {
        ...
        if (now < mKeepBeforeTime) {
            ...
            mHandler.sendEmptyMessageDelayed(RELEASE_CAMERA,
                    mKeepBeforeTime - now);
			Log.v(TAG, "-- release, delay ");
            return;
        }
        strongRelease();
		Log.v(TAG, "-- release, strong ");
    }

    public synchronized void strongRelease() {
        if (mCameraDevice == null) return;

		Log.v(TAG, "++ strongRelease");
        mCameraOpened = false;
        mCameraDevice.release();
        mCameraDevice = null;
