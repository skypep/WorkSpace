...
public class CameraDataAdapter implements LocalDataAdapter {
    ...
    public void addNewVideo(ContentResolver cr, Uri uri) {
        Cursor c = cr.query(uri,
                ...
                MediaStore.Video.Media.DATA + " like ? " 
                    	+ "or " + MediaStore.Video.Media.DATA + " like ? "
                    	+ "or " + MediaStore.Video.Media.DATA + " like ? "
                    , 
                getCameraPath(),
    ...
    private static String[] getCameraPath() {
        String[] cameraPath = {
			Storage.DIRECTORY + "/%", 
			Storage.DCIM + "/%",			// frankie, add 
            SDCard.instance().getDirectory() + "/%"
        };
        return cameraPath;
    }
    ...
    private class QueryTask extends AsyncTask<ContentResolver, Void, LocalDataList> {
        ...
        protected LocalDataList doInBackground(ContentResolver... resolver) {
            ...
            { // frankie, add 
            String[] data_paths = getCameraPath();
			Log.v(TAG, "data_paths.length: " + data_paths.length);
			for(String path : data_paths) {
				Log.v(TAG, "path: " + path);
			}
			}
			
            Cursor c = resolver[0].query(LocalMediaData.PhotoData.CONTENT_URI,
                    LocalMediaData.PhotoData.QUERY_PROJECTION,
                    //MediaStore.Images.Media.DATA + " like ? or " + MediaStore.Images.Media.DATA + " like ? ", 
                    MediaStore.Images.Media.DATA + " like ? " 
                    	+ "or " + MediaStore.Images.Media.DATA + " like ? "
