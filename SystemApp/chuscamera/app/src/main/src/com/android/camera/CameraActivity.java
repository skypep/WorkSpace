...
import com.android.camera.module.ScanModule;
import com.android.camera.module.ui.SlideTab1;
...
public class CameraActivity extends Activity
    ...
    public static final String ACTION_VOICE_MODULE_CAMERA_TAKE_PHOTO = "chushang.intent.action.camera.take.photo";                  // 相机模块(自拍)
    public static final String ACTION_VOICE_MODULE_CAMERA_TAKE_PA_PHOTO = "chushang.intent.action.camera.take.panoramic.photo";     // 相机模块(全景拍摄)
    ...
    /**
     */
    private static final String KEY_CS_URI = "key_cs_uri";
    /**
     */
    public static final String KEY_SECURE_CAMERA = "isSecureCamera";
    /**
     */
    public static final String KEY_SECURE_CAMERA_URIS = "SecureCameraUris";
    ...
    //private ScanModule mScanModule; // frankie, add 
    private CameraModule mPhotoModule;
    private CameraModule mVideoModule;
    private CameraModule mPanoModule;
    private CameraModule mScanModule;
    ...
    private View mCameraScanModuleRootView; 	// frankie, add 
	private SlideTab1 mSlideTab1 = null; 		// frankie, add 
	private RelativeLayout mSlideModeSwitcher = null; // frankie, add
    ...
    private ArrayList<String> mSeUris = new ArrayList<String>();
    ...
    private static final int REQUEST_GALLERY_CODE = 0x1000;//add

	// frankie, 2017.06.10, add start 
    private static final int LIGHTS_OUT_DELAY_MS = 4000;
    private final int BASE_SYS_UI_VISIBILITY =
            View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
			| View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION // frankie, 
            | View.SYSTEM_UI_FLAG_LAYOUT_STABLE;
    private final Runnable mLightsOutRunnable = new Runnable() {
        @Override
        public void run() {
            getWindow().getDecorView().setSystemUiVisibility(
                    BASE_SYS_UI_VISIBILITY
                            | View.SYSTEM_UI_FLAG_LOW_PROFILE
                    );
        }
    };
	// frankie, 2017.06.10, add end

	public TrickerControlBinder mTrickerControlBinder = null;

	public static Context mActivityContext = null;

    protected boolean mJumpGalley = false ;
    ...
    public void gotoGallery() {
        Log.v(TAG,"GOTOGallery:mSecureCamera:"+mSecureCamera);
        mJumpGalley = true;
        Log.w("SCAM_Activity", "mJumpGalley = true");
      /*  if(mThumbnail != null){
            mThumbnail.setClickable(false);
        }*/
        LocalDataAdapter adapter = getDataAdapter();
        ImageData img = adapter.getImageData(1);
        // remove by zj fix 1411
       /* if (img == null){
            return;
        }*/
        Uri uri = (img!=null)?img.getContentUri():null;
		Log.v(TAG, "uri: " + ((uri!=null)?uri.toString():""));
        if(mSecureCamera && mSeUris != null && mSeUris.size() == 0){
            uri = null;
        }
        ...
        // remove by zi fix 3580
/*		if (mCurrentModule instanceof com.android.camera.module.VideoModule) {	// frankie,
			// frankie, 2017.09.13, if img is video play it directly, or show it as photo
            if(!mSecureCamera){ // add zj fix 3359
                if(img != null && !img.isPhoto() && uri != null && !TextUtils.isEmpty(uri.toString())) {
                    Log.v(TAG, "gotoGallery, play video");
                    Intent intent = IntentHelper.getVideoPlayerIntent(this, uri);
                    //intent.setAction(Intent.ACTION_VIEW);
                    //intent.setData(uri);
                    intent.putExtra(KEY_FROM_SNAPCAM, true);
                    intent.putExtra(KEY_TOTAL_NUMBER, (adapter.getTotalNumber() - 1));
                    //intent.putExtra(Intent.EXTRA_TITLE, title);
                    intent.putExtra(CameraUtil.KEY_TREAT_UP_AS_BACK, true);
        ...
        try {
            Intent intent;
            if(!mSecureCamera && (img == null || uri == null || TextUtils.isEmpty(uri.toString()))){ // add zj for fix 1411
                Log.v(TAG,"getHomeIntent");
                intent = IntentHelper.getHomeIntent(this);
            }else {
                Log.v(TAG,"getGalleryIntent:mSecureCamera:"+mSecureCamera);
                intent = IntentHelper.getGalleryIntent(this);
            }

//            intent.setData(uri);
            intent.putExtra(KEY_CS_URI,(uri!=null?uri.toString():""));
            Log.v(TAG,"++++++++++++uri:"+((uri!=null)?uri.toString():""));
            intent.putExtra(KEY_SECURE_CAMERA,mSecureCamera);
            if(mSecureCamera){
                if(mSeUris != null && mSeUris.size() > 0){
                    intent.putStringArrayListExtra(KEY_SECURE_CAMERA_URIS,mSeUris);
                }
...
//              Intent intent = new Intent(Intent.ACTION_VIEW, uri);
    ...
    private void setSystemBarsVisibility(boolean visible, boolean hideLater) {
		...
		hidePreview = true; // frankie, we need navigation bar, but should set its background translucen !
    ...
    public void updateThumbnail_jpeg(final byte[] jpegData) {
		Log.v(TAG, "updateThumbnail_jpeg,");
    ...
    private Drawable getDefaultDra(){
        Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.chus_icon_thumbnail_default).copy(Bitmap.Config.ARGB_8888, true) ;
        ...
        array[1] =new BitmapDrawable(toRoundBitmap(bitmap));
        array[0] = new BitmapDrawable(bottomBitmap);
        LayerDrawable la = new LayerDrawable(array);
        la.setLayerInset(0, 0, 0, 0, 0);
        la.setLayerInset(1, 37, 37, 37, 37);
        return la;
    }
...
//            }
    ...
    public void updateThumbnail_ui_ready(ImageView thumbnail) {
    	Log.v(TAG, "updateThumbnail_ui_ready, ");
...
//            } else {
//                //mThumbnail.setVisibility(View.GONE);
//                mThumbnail.setVisibility(View.INVISIBLE); // frankie,
//            }
        }else if(!isCaptureIntent()){
            mThumbnail.setImageDrawable(getDefaultDra());
//            mThumbnail.setClickable(true);
        }
    }

    public void updateThumbnail__(boolean videoOnly) {
		Log.v(TAG, "updateThumbnail__(" + videoOnly + ")");
        ...
        if (!videoOnly 
			...
			|| (mCurrentModule instanceof com.android.camera.module.VideoModule) // frankie, add for our module
			) {
			Log.v(TAG, "  ... do update");
            (new UpdateThumbnailTask(null, true)).execute();
        }
		else {
			Log.w(TAG, "  ... NOT do update");
		}
    }
    ...
    private class UpdateThumbnailTask extends AsyncTask<Void, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Void... params) {
            ...
            if(mCurrentModule instanceof com.android.camera.module.PhotoModule){
                if(mSecureCamera && mSeUris != null && mSeUris.size() == 0)
                    return null;
            }
            LocalDataAdapter adapter = getDataAdapter();
...
//                    mThumbnail.setClickable(true);
    ...
    private class CircularDrawable extends Drawable {
        ...
        public CircularDrawable(Bitmap bitmap) {
            ...
            int targetSize = getResources().getDimensionPixelSize(R.dimen.chus_preview_thumbnail_size);
			mTargetSize_ = targetSize;
...
//            targetSize = getResources().getDimensionPixelSize(R.dimen.chus_preview_thumbnail_size);
//            Bitmap bottomBitmap = BitmapFactory.decodeResource(mActivityContext.getResources(), R.drawable.chus_ic_camera_switch_normal)
//                    .copy(Bitmap.Config.ARGB_8888, true) // copy a new one!
//                    ;
//            w = bottomBitmap.getWidth();
//            h = bottomBitmap.getHeight();
//            if (Math.min(w, h) < targetSize) {
//                Matrix matrix = new Matrix();
//                float scale = 1.0f;
//                if (w > h) {
...
//                }
//                matrix.postScale(scale, scale);
//                bottomBitmap = Bitmap.createBitmap(bottomBitmap, 0, 0, w, h, matrix, true);
//                w = (int) (w * scale);
//                h = (int) (h * scale);
//            }
//            if (w > h) {
//                mLength = h;
//                // public static Bitmap createBitmap(Bitmap source, int x, int y, int width, int height) {
//                bottomBitmap = Bitmap.createBitmap(bottomBitmap, (w - h) / 2, 0, h, h);
//            } else if (w < h) {
//                mLength = w;
//                // public static Bitmap createBitmap(Bitmap source, int x, int y, int width, int height) {
//                bottomBitmap = Bitmap.createBitmap(bottomBitmap, 0, (h - w) / 2, w, w);
//            } else {
//                mLength = w;
//            }

            mBitmapShader = new BitmapShader(bitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
            //mBitmapShader = new BitmapShader(bottomBitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
            mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            mPaint.setShader(mBitmapShader);

			//Log.v("yang", "targetSize: " + targetSize); // frankie,
        }
        ...
        protected void onBoundsChange(Rect bounds) {
			...
			//Log.v("yang", "onBoundsChange at (" + x + " * " + y + ")");
			mRect.left = x - mTargetSize_/2;
			mRect.right = x + mTargetSize_/2;
			mRect.top = y - mTargetSize_/2;
			mRect.bottom = y + mTargetSize_/2;
			//Log.v("yang", "onBoundsChange, " + (mRect.right - mRect.left)
			//	+ " * " + (mRect.bottom - mRect.top)); // frankie, 168*168
        }
	...
	// frankie, note, when photo/video/panorama save photo/video , use this to callback the adapter to update the thumbnail . 
    public void notifyNewMedia(Uri uri) {
		...
		Log.v(TAG, "notifyNewMedia,mimeType:" + mimeType);
        if (mimeType.startsWith("video/")) {
	...
	public void setBarTranslucent__() {
    ...
    private SlideTab1.Listener slideTab1Listener = new SlideTab1.Listener() { // frankie, add
        @Override
        public void onTextSelected(int index) {
            Log.v(TAG, "slideTab1Listener index:" + index);
            if(index == 0) {
				onModuleSelected(ModuleSwitcher.SCAN_MODULE_INDEX);
            } else if(index == 1) {
                onModuleSelected(ModuleSwitcher.PHOTO_MODULE_INDEX);
            } else if(index == 2) {
                onModuleSelected(ModuleSwitcher.VIDEO_MODULE_INDEX);
            } else if(index == 3) {
                onModuleSelected(ModuleSwitcher.WIDE_ANGLE_PANO_MODULE_INDEX);
            }
        }
    };
	public static final String KEY_REQUEST_PERMISSION_SECURE = "request_permission_secure__";
	private boolean mSecureCamera__i = false;
	protected void setSecureActivity__(boolean secure__) {
		mSecureCamera__i = secure__;
	}
        ...
        public void onReceive(Context context, Intent intent) {
            if (Intent.ACTION_USER_PRESENT.equals(intent.getAction())) {
                ...
                if(mDataAdapter != null){
                    Log.v(TAG,"unlock ,mDataAdapter.requestLoad");
    ...
    public void onCreate(Bundle state) {
		...
		// frankie, 2018.01.02, add start // disable secure camera first run permission request 
		if(true)
		//if(false) // frankie, debug to request non-critical permission
				// if request permission on secure camera, when the request returns , will back to normal Camera, not secure !!!
		{
			final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
			boolean isRequestShown = prefs.getBoolean(KEY_REQUEST_PERMISSION_SECURE, false);
			Log.v(TAG, "secure, +isRequestShown: " + isRequestShown);
			if(!isRequestShown) {
	            SharedPreferences.Editor editor = prefs.edit();
	            editor.putBoolean(KEY_REQUEST_PERMISSION_SECURE, true);
	            editor.apply();
			}
			isRequestShown = prefs.getBoolean(KEY_REQUEST_PERMISSION_SECURE, false);
			Log.v(TAG, "secure, -isRequestShown: " + isRequestShown);
		}
		// frankie, add end
		mTrickerControlBinder = new TrickerControlBinder(this); // frankie, 2018.01.30, add
		
        if (checkPermissions(mSecureCamera__i) || !mHasCriticalPermissions) {
        ...
        mCameraScanModuleRootView = rootLayout.findViewById(R.id.camera_scan_root);

		// frankie, add start
		mSlideModeSwitcher = (RelativeLayout)rootLayout.findViewById(R.id.slide_mode_switcher_1);
		mSlideTab1 = (SlideTab1)rootLayout.findViewById(R.id.slide_tab1);    
        mSlideTab1.setListener(slideTab1Listener);                          
		// frankie, add end
        ...
        if (MediaStore.INTENT_ACTION_VIDEO_CAMERA.equals(getIntent().getAction())
            ...
            if (prefs.getInt(CameraSettings.KEY_STARTUP_MODULE_INDEX, -1)
                ...
                || MediaStore.ACTION_IMAGE_CAPTURE_SECURE.equals(getIntent().getAction())) {
            moduleIndex = ModuleSwitcher.PHOTO_MODULE_INDEX;
        } else if(ACTION_VOICE_MODULE_CAMERA_TAKE_PA_PHOTO.equals(getIntent().getAction())){//add zj
            moduleIndex = ModuleSwitcher.WIDE_ANGLE_PANO_MODULE_INDEX;
        }else if(ACTION_VOICE_MODULE_CAMERA_TAKE_PHOTO.equals(getIntent().getAction())){//add zj
            moduleIndex = ModuleSwitcher.PHOTO_MODULE_INDEX;
        }else{
		...
		setBarTranslucent__(); // frankie, add 
...
//        super.onSaveInstanceState(outState);
    ...
    public void onPause() {
		...
		mTrickerControlBinder.onPause(); // frankie, 2018.01.30
        super.onPause();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQ_CODE_DONT_SWITCH_TO_PREVIEW) {
            ...
            if(resultCode == RESULT_OK) {
                ...
                if(jpeg != null) {
                    updateThumbnail_jpeg(jpeg);
                }
            }
        } else if(requestCode == REQUEST_GALLERY_CODE){
            if(resultCode == RESULT_OK) {
                ArrayList<String> l = data.getStringArrayListExtra(KEY_SECURE_CAMERA_URIS);
                if(l != null && l.size() > 0){
                    mSeUris.clear();
                    mSeUris.addAll(l);
                }else{
...
//                        mThumbnail.setClickable(true);
        ...
        }else if (mCurrentModule instanceof com.android.camera.module.PhotoModule) {
            ((com.android.camera.module.PhotoModule) mCurrentModule).onActivityResult(requestCode,resultCode,data);
        }else {
     ...
     */
    private boolean checkPermissions(boolean isSecure/* frankie, add */) {
    	Log.v(TAG, "checkPermissions, isSecure:" + isSecure);
		...
		// frankie, 2018.01.02, add start 
		// if secure camera, can continue without location permission !!!
		// but the first time the secure camera is open , still request the location permission,  fix this case !!!
		if(isSecure) {
			 final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
			 boolean isRequestShown = prefs.getBoolean(KEY_REQUEST_PERMISSION_SECURE, false);
			 Log.v(TAG, "secure,isRequestShown:" + isRequestShown + " mHasCriticalPermissions:" + mHasCriticalPermissions);
			 ...
			 Log.v(TAG, "secure,requestPermission: " + requestPermission);
		...
		Log.v(TAG, "  isRequestShown:" + isRequestShown + " mHasCriticalPermissions:" + mHasCriticalPermissions);
        if(!isRequestShown || !mHasCriticalPermissions) {
            Log.v(TAG, "Request permission");
            Intent intent = new Intent(this, PermissionsActivity.class);
            startActivity(intent);
		...
		Log.v(TAG, "requestPermission: " + requestPermission);
        return requestPermission;
    }

    @Override
    public void onResume() {
    	Log.v(TAG, "+++ onResume, hash:" + this.hashCode());
      /*  if(mThumbnail != null){
            mThumbnail.setClickable(true);
        }*/
        if (checkPermissions(mSecureCamera) || !mHasCriticalPermissions) {
            super.onResume();
			mTrickerControlBinder.onResume(); // frankie, 2018.01.30
...
//            }
    ...
    public void onDestroy() {
    	if(mCurrentModule != null) { // frankie, 2017.08.03, add
		...
		mTrickerControlBinder.onDestroy(); // frankie, 2018.01.30, add
        super.onDestroy();
    ...
    public void onBackPressed() {
    	Log.v("CAM_b", "CameraActivity, onBackPressed");
        if (!mFilmStripView.inCameraFullscreen()) {
            mFilmStripView.getController().goToFirstItem();
            mCurrentModule.resizeForPreviewAspectRatio();
        } else if (!mCurrentModule.onBackPressed()) {
        	this.finish();			// frankie, we finish directly, as super.onBackPressed() handles the fragment onBack
            //super.onBackPressed();
        }
    }
    ...
    /*protected*/public long updateStorageSpace() {
    ...
    /*protected*/public long getStorageSpaceBytes() { // frankie, 
        synchronized (mStorageSpaceLock) {
            return mStorageSpaceBytes;
        }
    }

    /*protected*/public void updateStorageSpaceAndHint() { // frankie, 
        updateStorageSpace();
        updateStorageHint(mStorageSpaceBytes);
    }

    /*protected*/public interface OnStorageUpdateDoneListener {
        void onStorageUpdateDone(long storageSpace);
    }

    /*protected*/public void updateStorageSpaceAndHint(final OnStorageUpdateDoneListener callback) { // frankie, 
    ...
    /*protected*/public void updateStorageHint(long storageSpace) {
        String message = null;
    ...
    /*protected*/public void setResultEx(int resultCode) { // frankie, 
        mResultCodeForTesting = resultCode;
        setResult(resultCode);
    }

    /*protected*/public void setResultEx(int resultCode, Intent data) { // frankie, 
        mResultCodeForTesting = resultCode;
	...
	public void onCameraIdToggled() {
		if(mCurrentModuleIndex == ModuleSwitcher.SCAN_MODULE_INDEX) { 
			if(mCurrentModule!=null) {mCurrentModule.onCameraIdToggle();}
		}
		...
		if(mCurrentModuleIndex == ModuleSwitcher.WIDE_ANGLE_PANO_MODULE_INDEX) {
			if(mCurrentModule!=null) {mCurrentModule.onCameraIdToggle();}
		}
	}
    ...
    public static final int SCAN_MODULE_INDEX = 10; // frankie, add scan module 

	private int[] module_next_map = new int[] {
		ModuleSwitcher.VIDEO_MODULE_INDEX, 	// PHOTO_MODULE_INDEX
		ModuleSwitcher.WIDE_ANGLE_PANO_MODULE_INDEX, // VIDEO_MODULE_INDEX
		...
		-1, //
		-1, // 
		ModuleSwitcher.PHOTO_MODULE_INDEX, // SCAN_MODULE_INDEX
	};
	...
	private int[] module_next_map_1 = new int[] {
		ModuleSwitcher.VIDEO_MODULE_INDEX, 	// PHOTO_MODULE_INDEX
		-1, // VIDEO_MODULE_INDEX
		ModuleSwitcher.PHOTO_MODULE_INDEX, // WIDE_ANGLE_PANO_MODULE_INDEX
		-1, // LIGHTCYCLE_MODULE_INDEX
		-1, // GCAM_MODULE_INDEX
		-1, // CAPTURE_MODULE_INDEX
		-1, //
	...
	public void onModuleSelectNext() { // frankie, add 
		if(AGlobalConfig.config_slidetab_reorder_en) {
			module_next_map = module_next_map_1;
			module_prev_map = module_prev_map_1;
		}
		int next = module_next_map[mCurrentModuleIndex];
		if(next != -1 && next != mCurrentModuleIndex) {
			Log.v(TAG, "onModuleSelectNext : " + next);
			onModuleSelected(next);
		}
	}
	public void onModuleSelectPrev() { // frankie, add 
		if(AGlobalConfig.config_slidetab_reorder_en) {
			module_next_map = module_next_map_1;
			module_prev_map = module_prev_map_1;
		}
		int prev = module_prev_map[mCurrentModuleIndex];
		if(prev != -1 && prev != mCurrentModuleIndex) {
			if(prev == ModuleSwitcher.SCAN_MODULE_INDEX) {
				if(AGlobalConfig.config_disable_scan_module_en) {
					return ;
				}
			}
			Log.v(TAG, "onModuleSelectPrev : " + prev);
			onModuleSelected(prev);
		}
	}
    @Override
    public void onModuleSelected(int moduleIndex) {
    	Log.v(TAG, "+ onModuleSelected:" + moduleIndex);
        boolean cam2on = mSettingsManager.isCamera2On();
        mForceReleaseCamera = moduleIndex == ModuleSwitcher.CAPTURE_MODULE_INDEX 
			|| (cam2on && moduleIndex == ModuleSwitcher.PHOTO_MODULE_INDEX);	// frankie, 
		Log.v(TAG, "mForceReleaseCamera:" + mForceReleaseCamera);
		...
		Log.v(TAG, "- onModuleSelected:");
    }

	private CameraModule  instantiateCameraModule_1(int module_index) {
		if(module_index == ModuleSwitcher.SCAN_MODULE_INDEX) {
			return new com.android.camera.module.ScanModule();
		}
		if(module_index == ModuleSwitcher.VIDEO_MODULE_INDEX) {
			return new com.android.camera.module.VideoModule();
		}
		if(module_index == ModuleSwitcher.WIDE_ANGLE_PANO_MODULE_INDEX) {
			return new com.android.camera.module.WideAnglePanoramaModule();
		}
		//if(module_index == ModuleSwitcher.PHOTO_MODULE_INDEX) {
			return new com.android.camera.module.PhotoModule();
		//}
	}
	private CameraModule  instantiateCameraModule_o(int module_index) {
		if(module_index == ModuleSwitcher.SCAN_MODULE_INDEX) {
	...
	private CameraModule  instantiateCameraModule(int module_index) {
		//return instantiateCameraModule_o(module_index);
		return instantiateCameraModule_1(module_index);
	}
    /**
    ...
    private void setModuleFromIndex(int moduleIndex) {
        ...
        mCameraScanModuleRootView.setVisibility(View.GONE); // frankie, add 
        mCurrentModuleIndex = moduleIndex;

		mSlideTab1.setIndex(moduleIndex); // frankie, add
        switch (moduleIndex) {
			case ModuleSwitcher.SCAN_MODULE_INDEX:
                if(mScanModule == null) {
                    mScanModule = instantiateCameraModule(moduleIndex);
                    mScanModule.init(this, mCameraScanModuleRootView);
                } else {
                    mScanModule.reinit();
                }
                mCurrentModule = mScanModule;
                mCameraScanModuleRootView.setVisibility(View.VISIBLE);
				break;
            case ModuleSwitcher.VIDEO_MODULE_INDEX:
                if(mVideoModule == null) {
                    mVideoModule = instantiateCameraModule(moduleIndex);
            ...
            case ModuleSwitcher.PHOTO_MODULE_INDEX:
                if(mPhotoModule == null) {
                    mPhotoModule = instantiateCameraModule(moduleIndex);
            ...
            case ModuleSwitcher.WIDE_ANGLE_PANO_MODULE_INDEX:
                if(mPanoModule == null) {
                    mPanoModule = instantiateCameraModule(moduleIndex);
            ...
            default:
                ...
                if(mPhotoModule == null) {
                    mPhotoModule = instantiateCameraModule(ModuleSwitcher.PHOTO_MODULE_INDEX);
                    mPhotoModule.init(this, mCameraPhotoModuleRootView);
                } else {
                    mPhotoModule.reinit();
                }
...
//    public boolean isRecording() { // frankie, not used rm .
...
//    }
	...
	// frankie, 2017.07.04 add start
	private ObjectAnimator showSlideSwitcherAnimator = null;
	private ObjectAnimator hideSlideSwitcherAnimator = null;
	public void setSlideSwitcherShow(boolean show, boolean force) { // when show=true, force should always be false
		if(mSlideModeSwitcher == null) {
			return ;
		}
        final View view_ = mSlideModeSwitcher;
        if(force) {
			if(showSlideSwitcherAnimator != null) {
                showSlideSwitcherAnimator.cancel();
                showSlideSwitcherAnimator = null;
			}
            if(hideSlideSwitcherAnimator != null) {
                hideSlideSwitcherAnimator.cancel();
                hideSlideSwitcherAnimator = null;
            }
            if(show) {
        ...
        if(show) {
            Log.v(TAG, "showSlideSwitcher"); // frankie,
            if (view_.getVisibility() != View.VISIBLE && showSlideSwitcherAnimator == null) {
                if (showSlideSwitcherAnimator != null) {
                    showSlideSwitcherAnimator.cancel();
                    showSlideSwitcherAnimator = null;
                }
                ObjectAnimator alphaAnimator = ObjectAnimator.ofFloat(view_, "alpha", 0f, 1f);
                ...
                alphaAnimator.start();
                showSlideSwitcherAnimator = alphaAnimator;
            }
        }
        else {
            Log.v(TAG, "hideSlideSwitcher"); // frankie,
            // Hide the preview cover if need.
            if (view_.getVisibility() != View.INVISIBLE && hideSlideSwitcherAnimator == null) {
                if (hideSlideSwitcherAnimator != null) {
                    hideSlideSwitcherAnimator.cancel();
                    hideSlideSwitcherAnimator = null;
                }

                ObjectAnimator alphaAnimator = ObjectAnimator.ofFloat(view_, "alpha", 1f, 0f);
                alphaAnimator.setDuration(300);
                alphaAnimator.addListener(new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationCancel(Animator animation) {
                ...
                alphaAnimator.start();
                hideSlideSwitcherAnimator = alphaAnimator;
            }
        }

		
	}
	// frankie, 2017.07.04 add end
