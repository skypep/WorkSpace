...
public class CameraEngine {
	final static String TAG = "filter_CameraEngine";
    ...
    private static void setDefaultParameters(){
		...
		Log.v(TAG, "previewSize : " + previewSize.width + "x" + previewSize.height);
        parameters.setPreviewSize(previewSize.width, previewSize.height);
		
        Size pictureSize = getLargePictureSize(camera);
		Log.v(TAG, "pictureSize : " + pictureSize.width + "x" + pictureSize.height);
        parameters.setPictureSize(pictureSize.width, pictureSize.height);

        int fps = parameters.getPreviewFrameRate();
        Log.v(TAG, "getPreviewFrameRate: " + fps);  // 30

        List<Integer> rates = parameters.getSupportedPreviewFrameRates(); // 15, 20, 24, 30
        for(Integer r : rates) {
            Log.v(TAG, "    " + r);
        }

        parameters.setRotation(90); // frankie, why set this ...
        camera.setParameters(parameters);
    }
    ...
    public static com.cs.camera.magicfilter.CameraInfo getCameraInfo(){
        com.cs.camera.magicfilter.CameraInfo info = new com.cs.camera.magicfilter.CameraInfo();
